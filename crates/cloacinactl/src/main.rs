...
use nouns::{
    accumulator, compiler, constructor, daemon, execution, graph, instance, key, package, reactor,
    secret, server, tenant, trigger, workflow,
};
...
enum Commands {
    ...
    /// Graph — computation graphs loaded in the server (health + pause state)
    Graph(graph::GraphCmd),

    /// Reactor — operator manual fire (force-fire / fire-with typed input)
    Reactor(reactor::ReactorCmd),

    /// Accumulator — operator manual event inject (typed JSON)
    Accumulator(accumulator::AccumulatorCmd),
    ...
    /// Instance — named, param-bound workflow instances
    Instance(instance::InstanceCmd),
...
async fn run() -> std::result::Result<(), CliError> {
    ...
    (match cli.command {
        ...
        Commands::Instance(cmd) => return cmd.run(&cli.globals).await,
