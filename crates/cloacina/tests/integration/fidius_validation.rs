...
fn test_plugin_info_populated() {
    ...
    assert_eq!(
        plugin.info.interface_version, 2,
        "Interface version should be 2 (CLOACI-I-0102 trait bump)"
    );
    assert_eq!(
        plugin.method_count, 6,
        "Should have 6 methods (get_task_metadata, execute_task, get_graph_metadata, execute_graph, get_reactor_metadata, get_trigger_metadata)"
    );
}
