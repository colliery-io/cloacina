...
impl KeyManager for DbKeyManager {
    async fn create_signing_key(
        ...
        let key_name_for_err = new_key.key_name.clone();
        ...
        .map_err(|e| {
            // Duplicate-name detection is backend-divergent in the raw string
            // (Postgres: "duplicate key…"; SQLite: "UNIQUE constraint…"). The OR
            // of both checks is a behavior-preserving superset for either backend.
            if e.to_string().contains("duplicate") || e.to_string().contains("UNIQUE") {
                KeyError::DuplicateName(key_name_for_err.clone())
            } else {
    ...
    async fn get_signing_key_info(
        ...
        let key: UnifiedSigningKey = crate::interact_on_backend!(self.dal, |conn| {
            signing_keys::table
    ...
    async fn revoke_signing_key(&self, key_id: UniversalUuid) -> Result<(), KeyError> {
        ...
        let now = UniversalTimestamp::now();
        let updated = crate::interact_on_backend!(self.dal, |conn| {
    ...
    async fn list_signing_keys(
        ...
        let keys: Vec<UnifiedSigningKey> = crate::interact_on_backend!(self.dal, |conn| {
            signing_keys::table
    ...
    async fn list_trusted_keys(
        ...
        let direct_keys: Vec<TrustedKeyInfo> = self.list_direct_trusted_keys(org_id).await?;

        // Get trusted child orgs via ACL
        let trusted_children: Vec<UniversalUuid> = self.get_trusted_child_orgs(org_id).await?;

        // Get keys from trusted children
        let mut all_keys = direct_keys;
        for child_org in trusted_children {
            let child_keys: Vec<TrustedKeyInfo> = self.list_direct_trusted_keys(child_org).await?;
    ...
    async fn find_trusted_key(
        ...
        let direct_key: Option<TrustedKeyInfo> =
            self.find_direct_trusted_key(org_id, fingerprint).await?;
        ...
        for child_org in trusted_children {
            let child_key: Option<TrustedKeyInfo> =
                self.find_direct_trusted_key(child_org, fingerprint).await?;
...
// Backend-agnostic query helpers (CLOACI-I-0135). These collapse the former
// `*_postgres`/`*_sqlite` twin pairs that were each called from more than one
// trait method; single-call twins were inlined directly into their trait method
// via `interact_on_backend!`.
impl DbKeyManager {
...
mod tests {
    ...
    async fn unique_dal() -> DAL {
        let url = format!(
            "file:dbkm_test_{}?mode=memory&cache=shared",
            uuid::Uuid::new_v4()
        );
