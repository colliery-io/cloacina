...
//! (CLOACI-I-0115 / S-0012, task T-0627; moved here from
//! `cloacina::delivery::envelope` in T-0642).
//!
//! A single versioned envelope consumed by every substrate WS consumer
...
impl ServerMessage {
    /// Build a `push` frame, base64-encoding the raw payload bytes.
    pub fn push(
        id: i64,
...
mod tests {
    ...
    fn push_round_trips_with_base64_payload() {
        let payload = b"\x00\xffhello".to_vec();
        let msg = ServerMessage::push(7, "work", "agent:abc", Some("t1".to_string()), &payload);
        let json = serde_json::to_string(&msg).unwrap();
        assert!(json.contains("\"type\":\"push\""));
        let back: ServerMessage = serde_json::from_str(&json).unwrap();
        assert_eq!(back.decode_push_payload().unwrap(), payload);
    }
