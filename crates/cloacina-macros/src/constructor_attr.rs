...
//! ## Kinds
//!
//! Each kind maps onto its own sync trait + body. All four are now
//! code-generated + fixture-proven: `task` (T-0826), `accumulator` / `reactor`
//! (T-0828), and `trigger` (T-0829).
//!
...
//!
//! Unlike `task` (which pulls `#[param]` fields from a context and writes outputs
//! via `set`/`get`), `accumulator` / `reactor` receive their payload as the body
//! argument and return the boundary / fire-context directly, so they take only
//! `#[config]` fields. See [`expand_event_kind`]. `trigger` ([`expand_trigger`])
//! has no incoming context either, so it is also `#[config]`-only, but it keeps
//! the `set`/`get` buffer to build the FIRE context. The consumer-side
//! `constructor!(...)` workflow instantiation (the runtime half of CLOACI-T-0829)
//! lives in `cloacina`'s `registry::loader::constructor_loader` +
//! `cloacina-macros`' `workflow_attr`.
...
fn expand_task(args: ConstructorArgs, item: DeriveInput) -> SynResult<TokenStream2> {
...
/// Code-gen for the TRIGGER kind (CLOACI-T-0829).
///
/// Mirrors [`expand_task`] but onto the trigger contract: the WASM method is
/// `poll` (not `execute`), the wire is [`TriggerInvocation`] in / [`PollOutcome`]
/// out, and the author body returns a FIRE DECISION rather than mutating a task
/// context. A trigger's `poll` is invoked by the host scheduler with NO incoming
/// task context (the `TriggerInvocation` envelope is empty), so — like
/// accumulator/reactor — `#[param]` is rejected; fields must all be `#[config]`.
///
/// The author writes ONLY:
///
/// ```rust,ignore
/// #[constructor(kind = trigger, name = "heartbeat", version = "0.1.0")]
/// struct Heartbeat {
///     #[config] should_fire: bool,
///     #[config] message: String,
/// }
/// impl Heartbeat {
///     // Ok(true) fires (with whatever was `set` into the fire context),
///     // Ok(false) skips this tick.
...
/// ```
///
/// `set`/`get` write the FIRE context (the analogue of the task output buffer):
/// on `Ok(true)` the accumulated outputs become the fired `PollOutcome.context_json`.
fn expand_trigger(args: ConstructorArgs, item: DeriveInput) -> SynResult<TokenStream2> {
    let struct_ident = item.ident.clone();
    let struct_vis = item.vis.clone();
    ...
    // A trigger's `poll` receives no task context, so (like accumulator/reactor)
    // all fields must be `#[config]`; `#[param]` is rejected.
    let mut config_fields: Vec<ConfigField> = Vec::new();
    let mut clean_field_defs: Vec<TokenStream2> = Vec::new();
    ...
    for field in &named.named {
        ...
        if has_param {
            return Err(syn::Error::new_spanned(
                field,
                "#[param] is only supported for `kind = task`; a trigger's `poll` \
                 receives no task context — use #[config] fields only",
            ));
        }
        if !is_config {
            return Err(syn::Error::new_spanned(
                field,
                "every trigger #[constructor] field must be marked #[config]",
            ));
        }
    ...
    // ----- The author struct, re-emitted clean + a fire-context output buffer. -----
    let outputs_field = quote! {
        __constructor_outputs:
            ::std::cell::RefCell<::serde_json::Map<::std::string::String, ::serde_json::Value>>
    };
    ...
    // Inherent `set` / `get` over the fire-context buffer (what the author body calls).
    let set_get_impl = quote! {
        impl #struct_ident {
            /// Write a key into the context this trigger fires with.
            #[allow(dead_code)]
            fn set(
                &self,
                key: impl ::std::convert::Into<::std::string::String>,
        ...
        impl #configured_ident {
            ...
            fn __constructor_run(&self, invocation_json: &str) -> #contract::PollOutcome {
                ...
                if let ::std::result::Result::Err(e) =
                    ::serde_json::from_str::<#contract::TriggerInvocation>(invocation_json)
                {
                    return #contract::PollOutcome::err(format!("decode invocation: {}", e));
                }

                // Build the author constructor: #[config] bound, fire-context empty.
                let __op = #struct_ident {
                    #(#config_binds,)*
                    __constructor_outputs: ::std::cell::RefCell::new(::serde_json::Map::new()),
                };

                // Run the ONLY thing the author wrote: the fire decision.
                match __op.poll() {
                    ::std::result::Result::Ok(true) => {
                        let __ctx = ::serde_json::Value::Object(__op.__constructor_outputs.into_inner());
                        match ::serde_json::to_string(&__ctx) {
                            ::std::result::Result::Ok(s) =>
                                #contract::PollOutcome::fire(::std::option::Option::Some(s)),
                            ::std::result::Result::Err(e) =>
                                #contract::PollOutcome::err(format!("encode fire context: {}", e)),
                        }
                    }
                    ::std::result::Result::Ok(false) => #contract::PollOutcome::skip(),
                    ::std::result::Result::Err(e) => #contract::PollOutcome::err(format!("{}", e)),
                }
            }
        }
    };
...
/// Code-gen for the ACCUMULATOR + REACTOR kinds (CLOACI-T-0828).
///
/// These differ from `task` in their I/O shape: the payload is an argument to
/// the author body (the event / the held boundaries), not pulled from a task
/// context, and the body returns the boundary/fire-context directly rather than
/// writing context keys. So there is no output buffer, no `set`/`get`, and
/// `#[param]` is rejected — fields must all be `#[config]` (bound once at load).
///
/// | kind          | trait                   | body fn    | author signature                                       |
/// |---------------|-------------------------|------------|--------------------------------------------------------|
/// | `accumulator` | `AccumulatorConstructor`| `ingest`   | `fn ingest(&self, event_json: &str)  -> Result<Option<String>, ConstructorError>` |
/// | `reactor`     | `ReactorConstructor`    | `evaluate` | `fn evaluate(&self, boundaries_json: &str) -> Result<Option<String>, ConstructorError>` |
///
/// For the accumulator, `Ok(Some(b))` emits boundary `b`, `Ok(None)` buffers.
/// For the reactor, `Ok(Some(ctx))` fires with context `ctx`, `Ok(None)` holds.
fn expand_event_kind(args: ConstructorArgs, item: DeriveInput) -> SynResult<TokenStream2> {
    let is_reactor = args.kind == Kind::Reactor;
    let struct_ident = item.ident.clone();
    let struct_vis = item.vis.clone();
    ...
    // Accumulator/reactor fields must all be #[config]: their input arrives as
    // the body's argument, so there is no context to pull `#[param]`s from.
    let mut config_fields: Vec<ConfigField> = Vec::new();
    let mut clean_field_defs: Vec<TokenStream2> = Vec::new();
    ...
    for field in &named.named {
        ...
        if has_param {
            return Err(syn::Error::new_spanned(
                field,
                "#[param] is only supported for `kind = task`; an accumulator/reactor \
                 receives its input as the body argument — use #[config] fields only",
            ));
        }
        if !is_config {
            return Err(syn::Error::new_spanned(
                field,
                "every accumulator/reactor #[constructor] field must be marked #[config]",
            ));
        }
    ...
    // ----- The author struct, re-emitted clean (config fields only). -----
    let clean_struct = quote! {
        #struct_vis struct #struct_ident {
            #(#clean_field_defs,)*
        }
    ...
    // Build the author struct per call, cloning #[config] out of the bound cfg.
    let config_binds = config_fields.iter().map(|c| {
        let id = &c.ident;
        quote! { #id: ::std::clone::Clone::clone(&self.cfg.#id) }
    });
    ...
    let invocation_ty = if is_reactor {
        quote! { #contract::ReactorInvocation }
    } else {
        quote! { #contract::AccumulatorInvocation }
    };
    let payload_field = if is_reactor {
        format_ident!("boundaries_json")
    } else {
        format_ident!("event_json")
    };
    let outcome_ty = if is_reactor {
        quote! { #contract::ReactorOutcome }
    } else {
        quote! { #contract::AccumulatorOutcome }
    };
    // Map the author body's `Result<Option<String>, ConstructorError>` onto the
    // kind's outcome, plus the decode-error + in-wasm encode-failure fallbacks.
    let (ok_some, ok_none, err_outcome, decode_err, encode_fallback) = if is_reactor {
        (
            quote! { #contract::ReactorOutcome { fire: true, context_json: ::std::option::Option::Some(__b), error: ::std::option::Option::None } },
            quote! { #contract::ReactorOutcome { fire: false, context_json: ::std::option::Option::None, error: ::std::option::Option::None } },
            quote! { #contract::ReactorOutcome { fire: false, context_json: ::std::option::Option::None, error: ::std::option::Option::Some(format!("{}", __e)) } },
            quote! { #contract::ReactorOutcome { fire: false, context_json: ::std::option::Option::None, error: ::std::option::Option::Some(format!("decode invocation: {}", __e)) } },
            r#"{{"fire":false,"error":"encode: {}"}}"#,
        )
    } else {
        (
            quote! { #contract::AccumulatorOutcome { boundary_json: ::std::option::Option::Some(__b), error: ::std::option::Option::None } },
            quote! { #contract::AccumulatorOutcome { boundary_json: ::std::option::Option::None, error: ::std::option::Option::None } },
            quote! { #contract::AccumulatorOutcome { boundary_json: ::std::option::Option::None, error: ::std::option::Option::Some(format!("{}", __e)) } },
            quote! { #contract::AccumulatorOutcome { boundary_json: ::std::option::Option::None, error: ::std::option::Option::Some(format!("decode invocation: {}", __e)) } },
            r#"{{"error":"encode: {}"}}"#,
        )
    };
    ...
    let guest_glue = quote! {
        ...
        impl #trait_ident for #configured_ident {
            fn #method_ident(&self, invocation_json: String) -> String {
                let __outcome = self.__constructor_run(&invocation_json);
                ::serde_json::to_string(&__outcome)
                    .unwrap_or_else(|e| format!(#encode_fallback, e))
            }
        }
        ...
        impl #configured_ident {
            ...
            fn __constructor_run(&self, invocation_json: &str) -> #outcome_ty {
                let __inv: #invocation_ty = match ::serde_json::from_str(invocation_json) {
                    ::std::result::Result::Ok(v) => v,
                    ::std::result::Result::Err(__e) => return #decode_err,
                };

                // Build the author constructor with #[config] bound, then run the
                // ONLY thing the author wrote.
                let __op = #struct_ident {
                    #(#config_binds,)*
                };
                match __op.#method_ident(&__inv.#payload_field) {
                    ::std::result::Result::Ok(::std::option::Option::Some(__b)) => #ok_some,
...
/// The Rust type NAME the manifest records for a `#[config]` field (CLOACI-T-0829):
/// the last path segment ident (e.g. `String`, `i64`, `bool`, `f64`). The consumer
/// uses it to bincode-encode the kwarg value as the concrete type the guest's
/// config struct decodes. Falls back to the rendered type tokens for exotic types.
fn config_type_name(ty: &Type) -> String {
    if let Type::Path(tp) = ty {
...
/// The `Vec<ConfigField>` literal recording a constructor's `#[config]` fields in
/// DECLARATION order (CLOACI-T-0829). Emitted into `__constructor_manifest()` so
/// the `constructor!` consumer can bind `config = { name = value }` by NAME.
fn config_fields_tokens(contract: &Path, config_fields: &[ConfigField]) -> TokenStream2 {
    let exprs = config_fields.iter().map(|c| {
        let name = c.ident.to_string();
        let ty = config_type_name(&c.ty);
        quote! { #contract::ConfigField { name: #name.to_string(), ty: #ty.to_string() } }
    });
    quote! { ::std::vec![ #(#exprs),* ] }
}
