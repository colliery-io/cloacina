...
//! ## Kinds
//!
//! Each kind maps onto its own sync trait + body. `task` (T-0826) and
//! `accumulator` / `reactor` (T-0828) are code-generated + fixture-proven;
//! `trigger` is authored against the raw fidius contract (the macro errors for it).
//!
...
//! `#[config]` fields. See [`expand_event_kind`]. The consumer-side
//! `constructor!(...)` workflow instantiation is a separate follow-up.
...
fn expand(args: ConstructorArgs, item: DeriveInput) -> SynResult<TokenStream2> {
    match args.kind {
        ...
        Kind::Trigger => Err(syn::Error::new(
            proc_macro2::Span::call_site(),
            "#[constructor]: `kind = trigger` is not code-generated by this macro; \
             author a trigger constructor against the raw fidius `TriggerConstructor` \
             contract (see trigger-constructor-fixture). task / accumulator / reactor \
             are code-generated.",
        )),
    }
}
