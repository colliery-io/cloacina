"""
Cloacina: Python bindings for the Cloacina workflow orchestration framework.

This package provides Python bindings for Cloacina, allowing you to define
and execute resilient task pipelines directly from Python code.

Installation:
    pip install cloacina[postgres]  # for PostgreSQL backend
    pip install cloacina[sqlite]    # for SQLite backend

Example:
    ```python
    from cloacina import task, Workflow, UnifiedExecutor

    @task(id="extract_data", dependencies=[])
    def extract_data(context):
        context["raw_data"] = {"users": [1, 2, 3]}
        return context

    @task(id="transform_data", dependencies=["extract_data"])
    def transform_data(context):
        raw = context.get("raw_data", {})
        context["transformed_data"] = {"processed": raw}
        return context

    # Create and run workflow
    workflow = Workflow("my_etl_pipeline")

    async def main():
        executor = UnifiedExecutor()
        await executor.initialize()
        await executor.execute(workflow)
        await executor.shutdown()

    import asyncio
    asyncio.run(main())
    ```
"""

# Try to import from available backend
try:
    from cloacina_postgres import (
        __version__,
        task,
        Workflow,
        UnifiedExecutor,
        TaskDecorator,
    )
    __backend__ = "postgres"
except ImportError:
    try:
        from cloacina_sqlite import (
            __version__,
            task,
            Workflow,
            UnifiedExecutor,
            TaskDecorator,
        )
        __backend__ = "sqlite"
    except ImportError:
        raise ImportError(
            "No Cloacina backend found. Please install with a backend:\n"
            "  pip install cloacina[postgres]\n"
            "  pip install cloacina[sqlite]"
        )

__all__ = [
    "__version__",
    "__backend__",
    "task",
    "Workflow", 
    "UnifiedExecutor",
    "TaskDecorator",
]