...
#[workflow(
    name = "acme_fulfillment",
    description = "Acme Corp order fulfillment — pick, pack, and ship open orders.",
    params(
        warehouse: String = "us-west",
        dry_run: bool = false,
    )
)]
pub mod acme_fulfillment_wf {
    use super::*;

    #[task]
    pub async fn ship_orders(context: &mut Context<serde_json::Value>) -> Result<(), TaskError> {
        let _ = context;
        Ok(())
    }
}
