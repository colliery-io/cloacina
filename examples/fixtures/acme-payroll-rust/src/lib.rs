...
#[workflow(
    name = "acme_payroll",
    description = "Acme Corp payroll run — compute hours and issue payments.",
    params(
        pay_period: String = "2026-06",
        dry_run: bool = false,
    )
)]
pub mod acme_payroll_wf {
    use super::*;

    #[task]
    pub async fn run_payroll(context: &mut Context<serde_json::Value>) -> Result<(), TaskError> {
        let _ = context;
        Ok(())
    }
}
