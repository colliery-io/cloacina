...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for task_result in &result.task_results {
        let (status_icon, display_status) = match &task_result.status {
            cloacina::task::TaskState::Completed { .. } => ("OK", "Completed"),
            cloacina::task::TaskState::Failed { .. } => ("FAIL", "Failed"),
            cloacina::task::TaskState::Pending => ("PEND", "Pending"),
            cloacina::task::TaskState::Running { .. } => ("RUN", "Running"),
            cloacina::task::TaskState::Skipped { .. } => ("SKIP", "Skipped"),
        };
