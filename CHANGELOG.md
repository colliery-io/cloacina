...
- **First-party Kafka event-source provider** (CLOACI-I-0139) — `cloacina-provider-kafka`, the flagship of the new **native provider** path: providers can now ship as native (non-WASM) per-architecture artifacts (`provider.json` with `runtime = "native"`, packaged via `cloacinactl constructor package --native`, opt-in with `features = ["native"]`), with artifacts for multiple architectures coexisting per provider version. Includes stream-accumulator wiring proven end-to-end against a real broker. Kafka support lives entirely in the provider — not in cloacina core. The provider crate ships in-repo in this release; crates.io publication of first-party providers follows on its own independent cadence (ADR A-0010, CLOACI-T-0871/0872).
- **Python runners are now safe to leak** (CLOACI-I-0140) — the `cloaca` module registers an `atexit` backstop that joins every live runner's runtime threads before interpreter finalization begins. Forgetting `runner.shutdown()` can no longer segfault interpreter teardown.
...
- **Examples are packaged-first** (CLOACI-I-0138) — the examples suite now leads with the packaged path (package → upload → compile → reconcile → execute) as the gold adoption story, with Python peers for the gold-path examples and the package→execute pipeline exercised in CI. Embedded examples remain first-class.
- **One authoring registration for `cloaca`** (CLOACI-I-0137) — the pip wheel and the embedded server now build the `cloaca` module from a single `register_authoring()` contract, making wheel/server symbol drift structurally impossible.
- **DAL backend twins collapsed** (CLOACI-I-0135) — the duplicated postgres/sqlite DAL implementations merged onto one backend-agnostic path (`interact_on_backend!`); only genuinely divergent pairs remain.
...
- **Execution-path reliability under transient DB contention** (CLOACI-I-0140) — a long-standing intermittent instability class (workflow executions hanging forever, tasks that succeeded reporting `Failed`, and interpreter-teardown segfaults in the Python bindings) was root-caused: transient database contention (sqlite busy/locked, postgres deadlock/serialization) on execution-path writes was being silently dropped. Terminal task-state writes, retry scheduling, the workflow-scheduling entry, status reads, and the delivery sweeper's redelivery reset now retry transient errors and never silently drop a state transition — a task can no longer be left `Running` forever, a succeeding task can no longer be failed by a lost context write, and a transient lock can no longer defer an outbox redelivery by a sweep interval. The GIL-safe `py_block_on` bridge (CLOACI-I-0136) and the runner `atexit` backstop harden the Python side of the same story.
- Rust-cdylib accumulator kinds carried the wrong type in FFI metadata, and the accumulator macros were unusable from packaged crates (CLOACI-T-0896).
