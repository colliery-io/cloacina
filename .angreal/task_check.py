...
def metrics_format():
    ...
    fixture as server-soak). Requires `promtool` on PATH — see the Prometheus
    release binaries if not installed.
    """
    import shutil
    import signal
    import time
    ...
    # Preflight — promtool must be installed.
    ...
    # Build the server binary (debug — fast rebuild).
    ...
    # Start Postgres via the shared compose fixture.
    ...
    # Wait for Postgres readiness (up to 30s).
    for _ in range(30):
    ...
    try:
        ...
        # Hit /health once more so the api_request_metrics middleware emits.
        try:
            urllib.request.urlopen("http://127.0.0.1:18181/health", timeout=2).read()
        except Exception:
            pass

        # Scrape /metrics (public route, no auth).
        ...
        # Pipe through `promtool check metrics`.
