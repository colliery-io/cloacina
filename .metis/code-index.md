...
> Generated: 2026-04-09T12:28:37Z | 423 files | JavaScript, Python, Rust
...
- pub `shutdown_signal` function L458-460 — `() -> (watch::Sender<bool>, watch::Receiver<bool>)` — Create a shutdown signal pair.
- pub `PollingAccumulator` interface L471-481 — `{ fn poll(), fn interval() }` — A polling accumulator periodically calls an async poll function to query
- pub `polling_accumulator_runtime` function L487-559 — `( mut poller: P, ctx: AccumulatorContext, socket_rx: mpsc::Receiver<Vec<u8>>, )` — Run a polling accumulator as a timer-based loop.
- pub `BatchAccumulator` interface L573-583 — `{ fn process_batch() }` — A batch accumulator buffers incoming events and processes them all at once
- pub `BatchAccumulatorConfig` struct L586-591 — `{ flush_interval: Option<std::time::Duration>, max_buffer_size: Option<usize> }` — Configuration for the batch accumulator runtime.
- pub `flush_signal` function L606-608 — `() -> (mpsc::Sender<()>, mpsc::Receiver<()>)` — Create a flush signal pair for batch accumulators.
- pub `batch_accumulator_runtime` function L615-701 — `( mut acc: B, ctx: AccumulatorContext, socket_rx: mpsc::Receiver<Vec<u8>>, mut f...` — Run a batch accumulator that buffers events and flushes on signal, timer, or size threshold.
- pub `StateAccumulator` struct L784-787 — `{ buffer: std::collections::VecDeque<T>, capacity: i32 }` — A state accumulator holds a bounded VecDeque<T> that receives values from
- pub `new` function L790-795 — `(capacity: i32) -> Self` — See CLOACI-S-0004 for the full specification.
- pub `state_accumulator_runtime` function L802-905 — `( mut acc: StateAccumulator<T>, ctx: AccumulatorContext, socket_rx: mpsc::Receiv...` — Run a state accumulator.
...
-  `accumulator_runtime_inner` function L363-455 — `( mut acc: A, ctx: AccumulatorContext, socket_rx: mpsc::Receiver<Vec<u8>>, confi...` — Inner runtime shared by both `accumulator_runtime` and `accumulator_runtime_with_source`.
-  `BatchAccumulatorConfig` type L593-600 — `impl Default for BatchAccumulatorConfig` — See CLOACI-S-0004 for the full specification.
-  `default` function L594-599 — `() -> Self` — See CLOACI-S-0004 for the full specification.
-  `persist_batch_buffer` function L704-715 — `(ctx: &AccumulatorContext, buffer: &[E])` — Persist batch buffer snapshot to DAL for crash resilience (best-effort).
-  `flush_batch` function L718-736 — `( acc: &mut B, buffer: &mut Vec<B::Event>, ctx: &AccumulatorContext, )` — Flush the buffer through the batch accumulator and send boundary if produced.
-  `set_health` function L743-747 — `(ctx: &AccumulatorContext, health: AccumulatorHealth)` — Set health state (best-effort, no-op if health channel not configured).
-  `persist_boundary` function L750-769 — `(ctx: &AccumulatorContext, boundary: &T)` — Persist last-emitted boundary with sequence number to DAL (best-effort, logs on failure).
-  `tests` module L908-1499 — `-` — See CLOACI-S-0004 for the full specification.
-  `TestEvent` struct L913-915 — `{ value: f64 }` — See CLOACI-S-0004 for the full specification.
-  `TestBoundary` struct L918-920 — `{ result: f64 }` — See CLOACI-S-0004 for the full specification.
-  `DoubleAccumulator` struct L922 — `-` — See CLOACI-S-0004 for the full specification.
-  `DoubleAccumulator` type L925-934 — `impl Accumulator for DoubleAccumulator` — See CLOACI-S-0004 for the full specification.
-  `Event` type L926 — `= TestEvent` — See CLOACI-S-0004 for the full specification.
-  `Output` type L927 — `= TestBoundary` — See CLOACI-S-0004 for the full specification.
-  `process` function L929-933 — `(&mut self, event: TestEvent) -> Option<TestBoundary>` — See CLOACI-S-0004 for the full specification.
-  `test_boundary_sender_round_trip` function L937-949 — `()` — See CLOACI-S-0004 for the full specification.
-  `test_accumulator_runtime_processes_socket_events` function L952-990 — `()` — See CLOACI-S-0004 for the full specification.
-  `test_accumulator_runtime_multiple_events` function L993-1029 — `()` — See CLOACI-S-0004 for the full specification.
-  `test_accumulator_shutdown` function L1032-1061 — `()` — See CLOACI-S-0004 for the full specification.
-  `CountingPoller` struct L1065-1068 — `{ count: u32, max: u32 }` — See CLOACI-S-0004 for the full specification.
-  `CountingPoller` type L1071-1088 — `impl PollingAccumulator for CountingPoller` — See CLOACI-S-0004 for the full specification.
-  `Output` type L1072 — `= TestBoundary` — See CLOACI-S-0004 for the full specification.
-  `poll` function L1074-1083 — `(&mut self) -> Option<TestBoundary>` — See CLOACI-S-0004 for the full specification.
-  `interval` function L1085-1087 — `(&self) -> std::time::Duration` — See CLOACI-S-0004 for the full specification.
-  `test_polling_accumulator_emits_on_some` function L1091-1128 — `()` — See CLOACI-S-0004 for the full specification.
-  `test_polling_accumulator_skips_on_none` function L1131-1160 — `()` — See CLOACI-S-0004 for the full specification.
-  `test_polling_accumulator_shutdown` function L1163-1187 — `()` — See CLOACI-S-0004 for the full specification.
-  `SumBatchAccumulator` struct L1191 — `-` — See CLOACI-S-0004 for the full specification.
-  `SumBatchAccumulator` type L1194-1202 — `impl BatchAccumulator for SumBatchAccumulator` — See CLOACI-S-0004 for the full specification.
-  `Event` type L1195 — `= TestEvent` — See CLOACI-S-0004 for the full specification.
-  `Output` type L1196 — `= TestBoundary` — See CLOACI-S-0004 for the full specification.
-  `process_batch` function L1198-1201 — `(&mut self, events: Vec<TestEvent>) -> Option<TestBoundary>` — See CLOACI-S-0004 for the full specification.
-  `test_batch_accumulator_flush_on_signal` function L1205-1254 — `()` — See CLOACI-S-0004 for the full specification.
-  `test_batch_accumulator_flush_on_timer` function L1257-1303 — `()` — See CLOACI-S-0004 for the full specification.
-  `test_batch_accumulator_empty_flush_skips` function L1306-1342 — `()` — See CLOACI-S-0004 for the full specification.
-  `test_batch_accumulator_max_buffer_size` function L1345-1390 — `()` — See CLOACI-S-0004 for the full specification.
-  `test_batch_accumulator_shutdown_drains` function L1393-1436 — `()` — See CLOACI-S-0004 for the full specification.
-  `FilterAccumulator` struct L1438 — `-` — See CLOACI-S-0004 for the full specification.
-  `FilterAccumulator` type L1441-1455 — `impl Accumulator for FilterAccumulator` — See CLOACI-S-0004 for the full specification.
-  `Event` type L1442 — `= TestEvent` — See CLOACI-S-0004 for the full specification.
-  `Output` type L1443 — `= TestBoundary` — See CLOACI-S-0004 for the full specification.
-  `process` function L1445-1454 — `(&mut self, event: TestEvent) -> Option<TestBoundary>` — See CLOACI-S-0004 for the full specification.
-  `test_accumulator_process_returns_none` function L1458-1498 — `()` — See CLOACI-S-0004 for the full specification.
...
- pub `ValidationError` enum L191-257 — `CyclicDependency | MissingDependency | MissingDependencyOld | CircularDependency...` — Errors that can occur during Workflow and dependency validation.
- pub `ExecutorError` enum L273-306 — `Database | ConnectionPool | TaskNotFound | TaskExecution | Context | TaskTimeout...` — Errors that can occur during task execution.
- pub `WorkflowError` enum L318-342 — `DuplicateTask | TaskNotFound | InvalidDependency | CyclicDependency | Unreachabl...` — Errors that can occur during workflow construction and management.
- pub `SubgraphError` enum L349-355 — `TaskNotFound | UnsupportedOperation` — Errors that can occur when creating Workflow subgraphs.
...
-  `ValidationError` type L259-263 — `= ValidationError` — relevant context information to aid in troubleshooting and recovery.
-  `from` function L260-262 — `(err: deadpool::managed::PoolError<deadpool_diesel::Error>) -> Self` — relevant context information to aid in troubleshooting and recovery.
-  `ContextError` type L265-269 — `= ContextError` — relevant context information to aid in troubleshooting and recovery.
-  `from` function L266-268 — `(err: deadpool::managed::PoolError<deadpool_diesel::Error>) -> Self` — relevant context information to aid in troubleshooting and recovery.
-  `ExecutorError` type L308-312 — `= ExecutorError` — relevant context information to aid in troubleshooting and recovery.
-  `from` function L309-311 — `(err: deadpool::managed::PoolError<deadpool_diesel::Error>) -> Self` — relevant context information to aid in troubleshooting and recovery.
-  `TaskError` type L358-381 — `= TaskError` — relevant context information to aid in troubleshooting and recovery.
-  `from` function L359-380 — `(error: ContextError) -> Self` — relevant context information to aid in troubleshooting and recovery.
...
- pub `prelude` module L453-483 — `-` — Prelude module for convenient imports.
- pub `computation_graph` module L487 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `context` module L488 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `cron_evaluator` module L489 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `cron_recovery` module L490 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `crypto` module L491 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `dal` module L492 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `database` module L493 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `dispatcher` module L494 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `error` module L495 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `executor` module L496 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `graph` module L497 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `logging` module L498 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `models` module L499 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `packaging` module L500 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `python` module L501 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `registry` module L502 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `retry` module L503 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `runner` module L504 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `scheduler` module L505 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `security` module L506 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `task` module L507 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `task_scheduler` module L508 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `trigger` module L509 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `workflow` module L510 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `setup_test` function L518-520 — `()` — - [`retry`]: Retry policies and backoff strategies
-  `cloaca` function L581-643 — `(m: &Bound<'_, PyModule>) -> PyResult<()>` — - [`retry`]: Retry policies and backoff strategies
...
-  `PipelineExecution` type L516-535 — `= PipelineExecution` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L517-534 — `(u: UnifiedPipelineExecution) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
...
-  `ThreadTaskExecutor` type L90-678 — `= ThreadTaskExecutor` — to the executor based on routing rules.
...
-  `handle_task_result` function L369-417 — `( &self, claimed_task: ClaimedTask, result: Result<Context<serde_json::Value>, E...` — Handles the result of task execution.
-  `save_task_context` function L427-457 — `( &self, claimed_task: &ClaimedTask, context: Context<serde_json::Value>, ) -> R...` — Saves the task's execution context to the database.
-  `mark_task_completed` function L466-487 — `( &self, task_execution_id: UniversalUuid, ) -> Result<(), ExecutorError>` — Marks a task as completed in the database.
-  `complete_task_transaction` function L503-527 — `( &self, claimed_task: &ClaimedTask, context: Context<serde_json::Value>, ) -> R...` — Completes a task by saving its context and marking it as completed.
-  `mark_task_failed` function L538-561 — `( &self, task_execution_id: UniversalUuid, error: &ExecutorError, ) -> Result<()...` — Marks a task as failed in the database.
-  `should_retry_task` function L577-614 — `( &self, claimed_task: &ClaimedTask, error: &ExecutorError, retry_policy: &Retry...` — Determines if a failed task should be retried.
-  `is_transient_error` function L623-640 — `(&self, error: &ExecutorError) -> bool` — Determines if an error is transient and potentially retryable.
-  `schedule_task_retry` function L650-677 — `( &self, claimed_task: &ClaimedTask, retry_policy: &RetryPolicy, ) -> Result<(),...` — Schedules a task for retry execution.
-  `ThreadTaskExecutor` type L680-694 — `impl Clone for ThreadTaskExecutor` — to the executor based on routing rules.
-  `clone` function L681-693 — `(&self) -> Self` — to the executor based on routing rules.
-  `ThreadTaskExecutor` type L701-983 — `impl TaskExecutor for ThreadTaskExecutor` — Implementation of the dispatcher's TaskExecutor trait.
-  `execute` function L702-962 — `(&self, event: TaskReadyEvent) -> Result<ExecutionResult, DispatchError>` — to the executor based on routing rules.
-  `has_capacity` function L964-966 — `(&self) -> bool` — to the executor based on routing rules.
-  `metrics` function L968-978 — `(&self) -> ExecutorMetrics` — to the executor based on routing rules.
-  `name` function L980-982 — `(&self) -> &str` — to the executor based on routing rules.
-  `tests` module L986-1224 — `-` — to the executor based on routing rules.
-  `test_merge_primitives_latest_wins` function L995-1000 — `()` — to the executor based on routing rules.
-  `test_merge_string_latest_wins` function L1003-1008 — `()` — to the executor based on routing rules.
-  `test_merge_different_types_latest_wins` function L1011-1016 — `()` — to the executor based on routing rules.
-  `test_merge_arrays_deduplicates` function L1019-1024 — `()` — to the executor based on routing rules.
-  `test_merge_arrays_no_overlap` function L1027-1032 — `()` — to the executor based on routing rules.
-  `test_merge_arrays_complete_overlap` function L1035-1040 — `()` — to the executor based on routing rules.
-  `test_merge_objects_no_conflict` function L1043-1048 — `()` — to the executor based on routing rules.
-  `test_merge_objects_conflicting_keys` function L1051-1056 — `()` — to the executor based on routing rules.
-  `test_merge_objects_recursive` function L1059-1064 — `()` — to the executor based on routing rules.
-  `test_merge_nested_arrays_in_objects` function L1067-1072 — `()` — to the executor based on routing rules.
-  `test_merge_null_latest_wins` function L1075-1080 — `()` — to the executor based on routing rules.
-  `test_merge_bool_latest_wins` function L1083-1088 — `()` — to the executor based on routing rules.
-  `sqlite_tests` module L1094-1223 — `-` — to the executor based on routing rules.
-  `test_executor` function L1097-1102 — `() -> ThreadTaskExecutor` — to the executor based on routing rules.
-  `test_is_transient_timeout` function L1105-1108 — `()` — to the executor based on routing rules.
-  `test_is_transient_task_not_found` function L1111-1114 — `()` — to the executor based on routing rules.
-  `test_is_transient_connection_pool` function L1117-1121 — `()` — to the executor based on routing rules.
-  `test_is_transient_task_execution_with_timeout_msg` function L1124-1133 — `()` — to the executor based on routing rules.
-  `test_is_transient_task_execution_permanent` function L1136-1145 — `()` — to the executor based on routing rules.
-  `test_is_transient_task_execution_network` function L1148-1157 — `()` — to the executor based on routing rules.
-  `test_is_transient_task_execution_unavailable` function L1160-1169 — `()` — to the executor based on routing rules.
-  `test_executor_has_capacity_initially` function L1176-1179 — `()` — to the executor based on routing rules.
-  `test_executor_metrics_initial` function L1182-1189 — `()` — to the executor based on routing rules.
-  `test_executor_name` function L1192-1195 — `()` — to the executor based on routing rules.
-  `test_executor_clone_shares_semaphore` function L1198-1206 — `()` — to the executor based on routing rules.
-  `test_executor_custom_config` function L1209-1222 — `()` — to the executor based on routing rules.
...
- pub `execute` function L1359-1396 — `( &self, workflow_name: &str, context: &PyContext, py: Python, ) -> PyResult<PyP...` — Execute a workflow by name with context
...
- pub `from_result` function L2127-2129 — `(result: crate::executor::PipelineResult) -> Self`
...
-  `PyPipelineResult` type L2126-2130 — `= PyPipelineResult`
...
-  `RegistryReconciler` type L27-845 — `= RegistryReconciler` — Package loading, unloading, and task/workflow registration.
-  `load_package` function L38-404 — `( &self, metadata: WorkflowMetadata, ) -> Result<(), RegistryError>` — Load a package into the global registries.
-  `unload_package` function L407-454 — `( &self, package_id: WorkflowPackageId, ) -> Result<(), RegistryError>` — Unload a package from the global registries
-  `register_package_tasks` function L457-498 — `( &self, metadata: &WorkflowMetadata, package_data: &[u8], ) -> Result<Vec<TaskN...` — Register tasks from a package into the global task registry
-  `register_package_workflows` function L501-642 — `( &self, metadata: &WorkflowMetadata, package_data: &[u8], ) -> Result<Option<St...` — Register workflows from a package into the global workflow registry
-  `create_workflow_from_host_registry` function L645-693 — `( &self, package_name: &str, workflow_name: &str, tenant_id: &str, ) -> Result<c...` — Create a workflow using the host's global task registry (avoiding FFI isolation)
-  `create_workflow_from_host_registry_static` function L696-743 — `( package_name: &str, workflow_name: &str, tenant_id: &str, ) -> Result<crate::w...` — Static version of create_workflow_from_host_registry for use in closures
-  `unregister_package_tasks` function L746-769 — `( &self, package_id: WorkflowPackageId, task_namespaces: &[TaskNamespace], ) -> ...` — Unregister tasks from the global task registry
...
- pub `RegistryReconciler` struct L160-185 — `{ registry: Arc<dyn WorkflowRegistry>, config: ReconcilerConfig, loaded_packages...` — Registry Reconciler for synchronizing database state with in-memory registries
- pub `new` function L189-212 — `( registry: Arc<dyn WorkflowRegistry>, config: ReconcilerConfig, shutdown_rx: wa...` — Create a new Registry Reconciler
- pub `with_reactive_scheduler` function L215-221 — `(self, scheduler: Arc<ReactiveScheduler>) -> Self` — Set the reactive scheduler for computation graph package routing.
- pub `set_reactive_scheduler_slot` function L225-230 — `( &mut self, slot: Arc<tokio::sync::RwLock<Option<Arc<ReactiveScheduler>>>>, )` — Replace the reactive scheduler slot with a shared reference from the runner.
- pub `start_reconciliation_loop` function L233-306 — `(mut self) -> Result<(), RegistryError>` — Start the background reconciliation loop
- pub `reconcile` function L309-421 — `(&self) -> Result<ReconcileResult, RegistryError>` — Perform a single reconciliation operation
- pub `get_status` function L447-462 — `(&self) -> ReconcilerStatus` — Get the current reconciliation status
...
-  `RegistryReconciler` type L187-463 — `= RegistryReconciler` — - `PackageState`: Tracking loaded package state
-  `shutdown_cleanup` function L424-444 — `(&self) -> Result<(), RegistryError>` — Perform cleanup operations during shutdown
-  `tests` module L466-649 — `-` — - `PackageState`: Tracking loaded package state
-  `test_reconciler_config_default` function L472-479 — `()` — - `PackageState`: Tracking loaded package state
-  `test_reconcile_result_methods` function L482-504 — `()` — - `PackageState`: Tracking loaded package state
-  `test_reconciler_status` function L507-531 — `()` — - `PackageState`: Tracking loaded package state
-  `test_reconciler_config_custom_values` function L534-548 — `()` — - `PackageState`: Tracking loaded package state
-  `test_reconcile_result_no_changes_no_failures` function L551-563 — `()` — - `PackageState`: Tracking loaded package state
-  `test_reconcile_result_unloaded_counts_as_change` function L566-577 — `()` — - `PackageState`: Tracking loaded package state
-  `test_reconcile_result_both_loaded_and_unloaded` function L580-594 — `()` — - `PackageState`: Tracking loaded package state
-  `test_package_status_detail_fields` function L597-609 — `()` — - `PackageState`: Tracking loaded package state
-  `test_reconciler_status_empty` function L612-620 — `()` — - `PackageState`: Tracking loaded package state
-  `test_reconciler_config_clone` function L623-632 — `()` — - `PackageState`: Tracking loaded package state
-  `test_reconcile_result_clone` function L635-648 — `()` — - `PackageState`: Tracking loaded package state
...
- pub `build` function L552-668 — `(self) -> Result<DefaultRunner, PipelineError>` — Builds the DefaultRunner
- pub `routing_config` function L686-689 — `(mut self, config: RoutingConfig) -> Self` — Sets custom routing configuration for task dispatch.
...
-  `DefaultRunnerBuilder` type L509-690 — `= DefaultRunnerBuilder` — configuring the DefaultRunner's behavior.
...
-  `tests` module L693-859 — `-` — configuring the DefaultRunner's behavior.
-  `test_default_runner_config` function L697-712 — `()` — configuring the DefaultRunner's behavior.
-  `test_registry_storage_backend_configuration` function L715-738 — `()` — configuring the DefaultRunner's behavior.
-  `test_runner_identification` function L741-749 — `()` — configuring the DefaultRunner's behavior.
-  `test_registry_configuration_options` function L752-773 — `()` — configuring the DefaultRunner's behavior.
-  `test_cron_configuration` function L776-791 — `()` — configuring the DefaultRunner's behavior.
-  `test_db_pool_size_default` function L794-797 — `()` — configuring the DefaultRunner's behavior.
-  `test_config_clone` function L800-813 — `()` — configuring the DefaultRunner's behavior.
-  `test_config_debug` function L816-824 — `()` — configuring the DefaultRunner's behavior.
-  `test_builder_all_fields` function L827-858 — `()` — configuring the DefaultRunner's behavior.
...
- pub `new` function L122-124 — `(database_url: &str) -> Result<Self, PipelineError>` — Creates a new default runner with default configuration
- pub `builder` function L138-140 — `() -> DefaultRunnerBuilder` — Creates a builder for configuring the executor
- pub `with_schema` function L158-164 — `(database_url: &str, schema: &str) -> Result<Self, PipelineError>` — Creates a new executor with PostgreSQL schema-based multi-tenancy
- pub `with_config` function L181-249 — `( database_url: &str, config: DefaultRunnerConfig, ) -> Result<Self, PipelineErr...` — Creates a new unified executor with custom configuration
...
- pub `shutdown` function L289-326 — `(&self) -> Result<(), PipelineError>` — Gracefully shuts down the executor and its background services
...
-  `DefaultRunner` type L37-405 — `= DefaultRunner` — the scheduler, executor, cron scheduler, cron recovery, and registry reconciler.
...
-  `start_registry_reconciler` function L258-357 — `( &self, handles: &mut super::RuntimeHandles, shutdown_tx: &broadcast::Sender<()...` — Starts the registry reconciler service
-  `start_stale_claim_sweeper` function L360-404 — `( &self, _handles: &mut super::RuntimeHandles, shutdown_tx: &broadcast::Sender<(...` — Starts the stale claim sweeper background service.
...
-  `process_pipelines_batch` function L200-251 — `( &self, active_executions: Vec<PipelineExecution>, ) -> Result<(), ValidationEr...` — Processes multiple pipelines in batch for better performance.
-  `dispatch_ready_tasks` function L258-286 — `(&self) -> Result<(), ValidationError>` — Dispatches all Ready tasks to the executor.
-  `complete_pipeline` function L289-342 — `( &self, execution: &PipelineExecution, ) -> Result<(), ValidationError>` — Completes a pipeline by updating its final context and marking it as completed.
...
-  `test_store_package_with_invalid_uuid` function L405-444 — `()`
-  `test_package_versioning` function L447-516 — `()`
...
-  `test_task_executor_timeout_handling` function L363-500 — `()`
-  `unified_task_test` function L506-510 — `(context: &mut Context<Value>) -> Result<(), TaskError>`
-  `test_default_runner_execution` function L513-635 — `()`
-  `initial_context_task_test` function L641-656 — `(context: &mut Context<Value>) -> Result<(), TaskError>`
-  `test_task_executor_context_loading_no_dependencies` function L659-805 — `()`
-  `producer_context_task` function L811-826 — `(context: &mut Context<Value>) -> Result<(), TaskError>`
-  `consumer_context_task` function L832-855 — `(context: &mut Context<Value>) -> Result<(), TaskError>`
-  `test_task_executor_context_loading_with_dependencies` function L858-1052 — `()`
...
-  `run_pipeline_and_get_status` function L1083-1177 — `( workflow_name: &str, task_defs: Vec<(&str, Box<dyn Fn() -> Arc<dyn Task> + Sen...` — Helper to set up a runner with registered tasks and workflow, execute, and
-  `test_pipeline_all_tasks_succeed_marked_completed` function L1182-1197 — `()` — COR-01: Pipeline where all tasks succeed must be marked "Completed".
-  `test_pipeline_task_fails_marked_failed` function L1202-1217 — `()` — COR-01: Pipeline where a task fails must be marked "Failed".
-  `test_pipeline_mixed_results_marked_failed` function L1222-1246 — `()` — COR-01: Pipeline with mixed results (one succeeds, one fails) must be "Failed".
-  `test_pipeline_skipped_downstream_marked_failed` function L1251-1275 — `()` — COR-01: Pipeline where a task fails and downstream tasks are skipped must be "Failed".
...
- pub `serialize` function L79-88 — `(value: &T) -> Result<Vec<u8>, GraphError>` — Serialize a value to bytes using the build-profile-appropriate format.
- pub `deserialize` function L91-100 — `(bytes: &[u8]) -> Result<T, GraphError>` — Deserialize bytes to a value using the build-profile-appropriate format.
- pub `InputCache` struct L115-117 — `{ entries: HashMap<SourceName, Vec<u8>> }` — The input cache holds the last-seen serialized boundary per source.
- pub `new` function L120-124 — `() -> Self` — this crate.
- pub `update` function L127-129 — `(&mut self, source: SourceName, bytes: Vec<u8>)` — Update the cached value for a source.
- pub `get` function L132-135 — `(&self, name: &str) -> Option<Result<T, GraphError>>` — Get and deserialize a cached value by source name.
- pub `has` function L138-140 — `(&self, name: &str) -> bool` — Check if a source has an entry in the cache.
- pub `get_raw` function L143-147 — `(&self, name: &str) -> Option<&[u8]>` — Get the raw bytes for a source.
- pub `snapshot` function L150-152 — `(&self) -> InputCache` — Create a snapshot (clone) of the cache.
- pub `len` function L155-157 — `(&self) -> usize` — Number of sources in the cache.
- pub `is_empty` function L160-162 — `(&self) -> bool` — Whether the cache is empty.
- pub `replace_all` function L165-167 — `(&mut self, other: InputCache)` — Replace all entries.
- pub `sources` function L170-172 — `(&self) -> Vec<&SourceName>` — List all source names in the cache.
- pub `entries_raw` function L175-177 — `(&self) -> &HashMap<SourceName, Vec<u8>>` — Get a reference to the raw entries map.
- pub `entries_as_json` function L180-194 — `(&self) -> HashMap<String, String>` — Return entries as a JSON-friendly map.
- pub `GraphResult` enum L213-218 — `Completed | Error` — Result of executing a compiled computation graph.
- pub `completed` function L221-223 — `(outputs: Vec<Box<dyn Any + Send>>) -> Self` — this crate.
- pub `completed_empty` function L225-229 — `() -> Self` — this crate.
- pub `error` function L231-233 — `(err: GraphError) -> Self` — this crate.
- pub `is_completed` function L235-237 — `(&self) -> bool` — this crate.
- pub `is_error` function L239-241 — `(&self) -> bool` — this crate.
- pub `GraphError` enum L246-261 — `Serialization | Deserialization | MissingInput | NodeExecution | Execution` — Errors that can occur during graph execution.
- pub `CompiledGraphFn` type L268-269 — `= Arc<dyn Fn(InputCache) -> Pin<Box<dyn Future<Output = GraphResult> + Send>> + ...` — Type alias for the compiled graph function.
- pub `ComputationGraphRegistration` struct L276-283 — `{ graph_fn: CompiledGraphFn, accumulator_names: Vec<String>, reaction_mode: Stri...` — Metadata about a registered computation graph.
- pub `ComputationGraphConstructor` type L285 — `= Box<dyn Fn() -> ComputationGraphRegistration + Send + Sync>` — this crate.
- pub `GlobalComputationGraphRegistry` type L286-287 — `= Arc<parking_lot::RwLock<HashMap<String, ComputationGraphConstructor>>>` — this crate.
- pub `register_computation_graph_constructor` function L293-300 — `(graph_name: String, constructor: F)` — Register a computation graph constructor in the global registry.
- pub `global_computation_graph_registry` function L303-305 — `() -> GlobalComputationGraphRegistry` — Get the global computation graph registry.
- pub `list_registered_graphs` function L308-311 — `() -> Vec<String>` — List all registered computation graph names.
- pub `deregister_computation_graph` function L314-318 — `(graph_name: &str)` — Remove a computation graph from the global registry.
- pub `types` module L321-323 — `-` — this crate.
...
-  `InputCache` type L119-195 — `= InputCache` — this crate.
-  `InputCache` type L197-201 — `impl Default for InputCache` — this crate.
-  `default` function L198-200 — `() -> Self` — this crate.
-  `hex_encode` function L203-205 — `(bytes: &[u8]) -> String` — this crate.
-  `GraphResult` type L220-242 — `= GraphResult` — this crate.
-  `GLOBAL_COMPUTATION_GRAPH_REGISTRY` variable L289-290 — `: once_cell::sync::Lazy<GlobalComputationGraphRegistry>` — this crate.
...
- pub `generate` function L49-347 — `(ir: &GraphIR, module: &ItemMod) -> syn::Result<TokenStream>` — Validate the graph against the module's functions and generate the compiled output.
-  `pascal_case_ident` function L33-46 — `(ident: &Ident) -> Ident` — Convert a snake_case Ident to PascalCase string for struct naming.
-  `extract_functions` function L350-368 — `(module: &ItemMod) -> syn::Result<HashMap<String, ItemFn>>` — Extract named async functions from a module.
-  `has_blocking_attr` function L371-380 — `(func: &ItemFn) -> bool` — Check if a function has `#[node(blocking)]` attribute.
-  `generate_compiled_function` function L386-438 — `( ir: &GraphIR, functions: &HashMap<String, ItemFn>, blocking_nodes: &HashSet<St...` — Generate the body of the compiled async function.
-  `generate_cache_reads` function L441-458 — `(ir: &GraphIR) -> TokenStream` — Generate `let` bindings for cache reads.
-  `generate_node_execution` function L461-538 — `( ir: &GraphIR, node: &GraphNode, functions: &HashMap<String, ItemFn>, blocking_...` — Generate execution code for a single node.
-  `generate_call_args` function L541-568 — `(ir: &GraphIR, node: &GraphNode) -> TokenStream` — Generate the argument list for a node function call.
-  `generate_routing_match` function L571-619 — `( ir: &GraphIR, from_name: &str, variants: &[super::graph_ir::GraphRoutingVarian...` — Generate match arms for a routing node.
-  `generate_routing_use_stmts` function L623-651 — `( ir: &GraphIR, functions: &HashMap<String, ItemFn>, mod_name: &Ident, ) -> Vec<...` — Generate `use ModName::ReturnType::*;` for routing nodes so enum variant
...
- pub `AppState` struct L38-49 — `{ database: Database, runner: Arc<DefaultRunner>, key_cache: Arc<crate::server::...` — Shared application state accessible from all route handlers.
- pub `run` function L52-251 — `( home: std::path::PathBuf, bind: SocketAddr, database_url: String, verbose: boo...` — Run the API server.
- pub `RequestId` struct L259 — `-` — Build the axum router with all routes.
-  `request_id_middleware` function L263-284 — `( mut request: axum::extract::Request, next: axum::middleware::Next, ) -> axum::...` — Middleware that generates a UUID request ID, creates a tracing span,
-  `build_router` function L286-417 — `(state: AppState) -> Router` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `health` function L420-422 — `() -> impl IntoResponse` — GET /health — liveness check (no auth, no DB)
-  `ready` function L425-454 — `(State(state): State<AppState>) -> impl IntoResponse` — GET /ready — readiness check (verifies DB connection pool is healthy)
-  `metrics` function L457-467 — `(State(state): State<AppState>) -> impl IntoResponse` — GET /metrics — Prometheus metrics (placeholder for now)
-  `fallback_404` function L470-475 — `() -> impl IntoResponse` — Fallback for unmatched routes — returns 404 JSON
-  `shutdown_signal` function L478-500 — `()` — Wait for shutdown signal (SIGINT or SIGTERM)
-  `bootstrap_admin_key` function L506-554 — `( state: &AppState, home: &std::path::Path, provided_key: Option<&str>, ) -> Res...` — Bootstrap: create an admin API key on first startup if none exist.
-  `mask_db_url` function L558-560 — `(url: &str) -> String` — Mask password in database URL for logging
-  `tests` module L563-1466 — `-` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `TEST_DB_URL` variable L571 — `: &str` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_state` function L574-605 — `() -> AppState` — Create a test AppState with a real Postgres connection.
-  `create_test_api_key` function L608-616 — `(state: &AppState) -> String` — Create a bootstrap API key and return the plaintext token.
-  `send_request` function L619-634 — `( app: Router, request: axum::http::Request<Body>, ) -> (StatusCode, serde_json:...` — Send a request to the router and return (status, body as serde_json::Value).
-  `test_request_id_header_present` function L640-666 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_health_returns_200` function L672-684 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_ready_returns_200_with_db` function L688-700 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_metrics_returns_prometheus_format` function L704-757 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_auth_no_token_returns_401` function L763-775 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_auth_invalid_token_returns_401` function L779-792 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_auth_valid_token_passes` function L796-809 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_auth_malformed_header_returns_401` function L813-826 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_create_key_returns_201` function L832-850 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_create_key_missing_name_returns_422` function L854-870 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_list_keys_returns_list` function L874-889 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_revoke_key_valid` function L893-918 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_revoke_key_nonexistent_returns_404` function L922-937 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_revoke_key_invalid_uuid_returns_400` function L941-955 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_create_tenant_returns_201` function L961-987 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_list_tenants` function L991-1005 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_remove_tenant_nonexistent_succeeds` function L1009-1025 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_create_then_delete_tenant` function L1029-1066 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_create_tenant_missing_fields_returns_422` function L1070-1085 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_list_workflows_returns_list` function L1091-1105 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_get_workflow_nonexistent_returns_404` function L1109-1122 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_upload_workflow_empty_file_returns_400` function L1126-1150 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_upload_workflow_no_file_field_returns_400` function L1154-1178 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `fixture_path` function L1181-1186 — `(name: &str) -> std::path::PathBuf` — Path to test fixture directory (relative to workspace root).
-  `multipart_file_body` function L1189-1200 — `(data: &[u8]) -> (String, Vec<u8>)` — Build a multipart request body with a file field.
-  `delete_workflow_if_exists` function L1203-1213 — `(state: &AppState, token: &str, name: &str, version: &str)` — Delete a workflow by name/version if it exists (cleanup for idempotent tests).
-  `test_upload_valid_python_workflow_returns_201` function L1217-1243 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_upload_valid_rust_workflow_returns_201` function L1247-1273 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_upload_corrupt_package_returns_400` function L1277-1297 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_list_executions_returns_list` function L1303-1317 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_get_execution_invalid_uuid_returns_400` function L1321-1334 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_get_execution_nonexistent_returns_404` function L1338-1352 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_get_execution_events_invalid_uuid_returns_400` function L1356-1369 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_execute_nonexistent_workflow_returns_error` function L1373-1388 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_get_execution_events_valid_uuid_no_events` function L1392-1410 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_list_triggers_returns_list` function L1416-1430 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_get_trigger_nonexistent_returns_404` function L1434-1447 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
-  `test_unknown_route_returns_404` function L1453-1465 — `()` — Later tasks add auth, tenant management, workflow upload, and execution APIs.
...
- pub `too_many_requests` function L77-79 — `(message: impl Into<String>) -> Self` — error responses with request correlation IDs.
-  `ApiError` type L45-80 — `= ApiError` — error responses with request correlation IDs.
-  `ApiError` type L82-90 — `impl IntoResponse for ApiError` — error responses with request correlation IDs.
-  `into_response` function L83-89 — `(self) -> Response` — error responses with request correlation IDs.
...
- pub `revoke_key` function L135-165 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — DELETE /auth/keys/:key_id — revoke an API key.
- pub `create_tenant_key` function L169-211 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — POST /tenants/:tenant_id/keys — create a key scoped to a tenant.
- pub `create_ws_ticket` function L217-227 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, )...` — POST /auth/ws-ticket — exchange a Bearer token for a single-use WebSocket ticket.
...
- pub `upload_workflow` function L36-110 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — POST /tenants/:tenant_id/workflows — multipart upload of .cloacina source package.
- pub `list_workflows` function L113-154 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — GET /tenants/:tenant_id/workflows — list registered workflows.
- pub `get_workflow` function L157-195 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — GET /tenants/:tenant_id/workflows/:name — get workflow details.
- pub `delete_workflow` function L198-240 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — DELETE /tenants/:tenant_id/workflows/:name/:version — unregister workflow.
