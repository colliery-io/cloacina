...
> Generated: 2026-04-04T20:26:32Z | 390 files | JavaScript, Python, Rust
...
- pub `types` module L26 — `-` — # Computation Graph Runtime Types
...
-  `cloaca` function L571-613 — `(m: &Bound<'_, PyModule>) -> PyResult<()>` — - [`retry`]: Retry policies and backoff strategies
...
- pub `computation_graph_attr` function L33-41 — `(args: TokenStream, input: TokenStream) -> TokenStream` — The `#[computation_graph]` attribute macro entry point.
-  `codegen` module L22 — `-` — `#[computation_graph]` attribute macro implementation.
-  `graph_ir` module L23 — `-` — validates it, and generates a compiled async function.
-  `parser` module L24 — `-` — validates it, and generates a compiled async function.
-  `computation_graph_impl` function L43-59 — `( args: proc_macro2::TokenStream, input: proc_macro2::TokenStream, ) -> syn::Res...` — validates it, and generates a compiled async function.
