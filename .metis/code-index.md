...
- pub `components` interface L426-855 — `{ schemas: : { /** @description One row in `GET /v1/health/accumulators`. */ Acc...`
- pub `$defs` type L856 — `= Record<string, never>`
- pub `operations` interface L857-2009 — `{ health: : { parameters: { query?: never; header?: never; path?: never; cookie?...`
...
- pub `schemas` type L21 — `= components["schemas"]`
- pub `ErrorBody` type L23 — `= schemas["ErrorBody"]`
- pub `ListExecutionsQuery` type L26-28 — `= NonNullable< paths["/v1/tenants/{tenant_id}/executions"]["get"]["parameters"][...`
- pub `ListTriggersQuery` type L31-33 — `= NonNullable< paths["/v1/tenants/{tenant_id}/triggers"]["get"]["parameters"]["q...`
- pub `CloacinaApiError` class L36-46 — `extends Error`
- pub `constructor` method L40-45 — `constructor(status: number, body: ErrorBody | undefined)`
- pub `CloacinaClientOptions` interface L48-64 — `{ baseUrl: : string, apiKey: : string, tenant: : string, fetch: : typeof globalT...`
- pub `CloacinaClient` class L84-397 — `-`
- pub `constructor` method L91-108 — `constructor(options: CloacinaClientOptions)`
- pub `withTenant` method L111-117 — `withTenant(tenant: string): CloacinaClient`
- pub `health` method L131-133 — `health(): Promise<unknown>`
- pub `ready` method L135-140 — `ready(): Promise<Response>`
- pub `createKey` method L144-148 — `createKey( body: schemas["CreateKeyRequest"], ): Promise<schemas["KeyCreatedResp...`
- pub `listKeys` method L150-152 — `listKeys(): Promise<schemas["ListResponse_KeyInfo"]>`
- pub `revokeKey` method L154-160 — `revokeKey(keyId: string): Promise<schemas["KeyRevokedResponse"]>`
- pub `createTenantKey` method L162-172 — `createTenantKey( body: schemas["CreateKeyRequest"], tenant?: string, ): Promise<...`
- pub `createWsTicket` method L175-177 — `createWsTicket(): Promise<schemas["WsTicketResponse"]>`
- pub `createTenant` method L181-185 — `createTenant( body: schemas["CreateTenantRequest"], ): Promise<schemas["TenantCr...`
- pub `listTenants` method L187-189 — `listTenants(): Promise<schemas["ListResponse_TenantSummary"]>`
- pub `removeTenant` method L191-199 — `removeTenant( schemaName: string, ): Promise<schemas["TenantRemovedResponse"]>`
- pub `uploadWorkflow` method L207-229 — `uploadWorkflow( pkg: Blob | Uint8Array, tenant?: string, ): Promise<schemas["Wor...`
- pub `listWorkflows` method L231-239 — `listWorkflows( tenant?: string, ): Promise<schemas["TenantListResponse_WorkflowS...`
- pub `getWorkflow` method L241-250 — `getWorkflow( name: string, tenant?: string, ): Promise<schemas["WorkflowDetail"]...`
- pub `deleteWorkflow` method L252-267 — `deleteWorkflow( name: string, version: string, tenant?: string, ): Promise<schem...`
- pub `listTriggers` method L271-283 — `listTriggers( query?: ListTriggersQuery, tenant?: string, ): Promise<schemas["Te...`
- pub `getTrigger` method L285-294 — `getTrigger( name: string, tenant?: string, ): Promise<schemas["TriggerDetailResp...`
- pub `executeWorkflow` method L298-312 — `executeWorkflow( name: string, body: schemas["ExecuteRequest"] = {}, tenant?: st...`
- pub `listExecutions` method L314-326 — `listExecutions( query?: ListExecutionsQuery, tenant?: string, ): Promise<schemas...`
- pub `iterateExecutions` method L332-351 — `iterateExecutions( query: Omit<ListExecutionsQuery, "offset"> = {}, tenant?: str...`
- pub `getExecution` method L353-362 — `getExecution( execId: string, tenant?: string, ): Promise<schemas["ExecutionDeta...`
- pub `getExecutionEvents` method L364-378 — `getExecutionEvents( execId: string, tenant?: string, ): Promise<schemas["Executi...`
- pub `listAccumulators` method L382-384 — `listAccumulators(): Promise<schemas["ListResponse_AccumulatorStatus"]>`
- pub `listGraphs` method L386-388 — `listGraphs(): Promise<schemas["ListResponse_GraphStatus"]>`
- pub `getGraph` method L390-396 — `getGraph(name: string): Promise<schemas["GraphStatus"]>`
-  `unwrap` function L66-75 — `function unwrap(result: { data?: T; error?: ErrorBody; response: Response; }): T`

#### clients/typescript/src/ws.ts

- pub `DeliveryWelcome` interface L33-37 — `{ type: : "welcome", protocol_version: : number, max_known_id: : number }`
- pub `DeliveryPush` interface L39-47 — `{ type: : "push", protocol_version: : number, id: : number, kind: : string, reci...`
- pub `DeliveryServerMessage` type L49 — `= DeliveryWelcome | DeliveryPush`
- pub `WebSocketLike` interface L52-56 — `{ send(), close(), addEventListener() }`
- pub `WebSocketConstructor` type L58 — `= new (url: string) => WebSocketLike`
- pub `DeliverySubscribeOptions` interface L60-81 — `{ signal: : AbortSignal, webSocket: : WebSocketConstructor, reconnect: : boolean...`
- pub `decodePushPayload` function L97-99 — `function decodePushPayload(push: DeliveryPush): Uint8Array`
- pub `decodePushJson` function L102-104 — `function decodePushJson(push: DeliveryPush): unknown`
-  `decodeBase64` function L83-94 — `function decodeBase64(b64: string): Uint8Array`
-  `QueueWaiter` interface L106-109 — `{ resolve: : (value: IteratorResult<DeliveryPush>) => void, reject: : (err: unkn...`
-  `abort` function L159 — `const abort = ()`
-  `nextFrame` function L210-223 — `const nextFrame = (): Promise<IteratorResult<DeliveryPush>>`
...
- pub `ValidationError` enum L191-260 — `CyclicDependency | MissingDependency | DuplicateTaskId | EmptyWorkflow | Invalid...` — Errors that can occur during Workflow and dependency validation.
- pub `ExecutorError` enum L276-315 — `Database | ConnectionPool | TaskNotFound | TaskExecution | Context | TaskTimeout...` — Errors that can occur during task execution.
- pub `WorkflowError` enum L327-351 — `DuplicateTask | TaskNotFound | InvalidDependency | CyclicDependency | Unreachabl...` — Errors that can occur during workflow construction and management.
- pub `SubgraphError` enum L358-364 — `TaskNotFound | UnsupportedOperation` — Errors that can occur when creating Workflow subgraphs.
...
-  `ValidationError` type L262-266 — `= ValidationError` — relevant context information to aid in troubleshooting and recovery.
...
-  `ContextError` type L268-272 — `= ContextError` — relevant context information to aid in troubleshooting and recovery.
-  `from` function L269-271 — `(err: deadpool::managed::PoolError<deadpool_diesel::Error>) -> Self` — relevant context information to aid in troubleshooting and recovery.
-  `ExecutorError` type L317-321 — `= ExecutorError` — relevant context information to aid in troubleshooting and recovery.
-  `from` function L318-320 — `(err: deadpool::managed::PoolError<deadpool_diesel::Error>) -> Self` — relevant context information to aid in troubleshooting and recovery.
-  `TaskError` type L367-390 — `= TaskError` — relevant context information to aid in troubleshooting and recovery.
-  `from` function L368-389 — `(error: ContextError) -> Self` — relevant context information to aid in troubleshooting and recovery.
...
- pub `delivery` module L510 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `dispatcher` module L511 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `error` module L512 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `execution_planner` module L515 — `-` — Task readiness evaluation, workflow processing, and stale claim sweeping.
- pub `executor` module L516 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `fleet` module L517 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `graph` module L518 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `inventory_entries` module L519 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `logging` module L520 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `models` module L521 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `packaging` module L522 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `python_runtime` module L523 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `registry` module L524 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `retry` module L525 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `runner` module L526 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `runtime` module L527 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `security` module L532 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `task` module L533 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `trigger` module L534 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `var` module L535 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `workflow` module L536 — `-` — - [`retry`]: Retry policies and backoff strategies
- pub `setup_test` function L545-547 — `()` — - [`retry`]: Retry policies and backoff strategies
...
#### crates/cloacina/src/dal/unified/delivery_outbox.rs

- pub `DeliveryOutboxDAL` struct L50-52 — `{ dal: &'a DAL }` — Data access layer for delivery-outbox operations with runtime backend selection.
- pub `new` function L70-72 — `(dal: &'a DAL) -> Self` — schema parity and test coverage.
- pub `enqueue` function L81-87 — `(&self, new: NewDeliveryOutbox) -> Result<DeliveryOutbox, ValidationError>` — Enqueues a new outbox row in the `pending` state.
- pub `mark_delivered` function L139-146 — `(&self, id: i64) -> Result<(), ValidationError>` — `pending → delivered`: record that the row was pushed to its recipient.
- pub `mark_acked` function L212-219 — `(&self, id: i64) -> Result<(), ValidationError>` — `pending|delivered → acked`: recipient confirmed receipt.
- pub `reset_to_pending` function L274-281 — `(&self, id: i64) -> Result<(), ValidationError>` — `delivered → pending`: redelivery path (sweeper reclaim / reconnect resync).
- pub `reset_delivered_to_pending_for_recipient` function L334-348 — `( &self, recipient: &str, tenant_id: Option<&str>, ) -> Result<usize, Validation...` — Reset every `delivered`-state row addressed to `(recipient, tenant_id)`
- pub `reassign_open_rows` function L426-440 — `( &self, from_recipient: &str, to_recipient: &str, ) -> Result<usize, Validation...` — Reassign every non-`acked` row from `from_recipient` to `to_recipient`,
- pub `list_open_for_recipient` function L508-520 — `( &self, recipient: &str, limit: i64, ) -> Result<Vec<DeliveryOutbox>, Validatio...` — Open (un-acked) rows addressed to `recipient`, ordered by id for
- pub `list_pending` function L577-583 — `(&self, limit: i64) -> Result<Vec<DeliveryOutbox>, ValidationError>` — Rows in the `pending` state across all recipients, oldest first — the
- pub `list_stuck` function L635-645 — `( &self, cutoff: UniversalTimestamp, limit: i64, ) -> Result<Vec<DeliveryOutbox>...` — Open rows enqueued before `cutoff`, oldest first — the safety-net
- pub `count_open` function L700-706 — `(&self) -> Result<i64, ValidationError>` — Count of open (un-acked) rows — outbox-depth signal for monitoring (T-0628).
-  `STATE_PENDING` variable L44 — `: &str` — schema parity and test coverage.
-  `STATE_DELIVERED` variable L45 — `: &str` — schema parity and test coverage.
-  `STATE_ACKED` variable L46 — `: &str` — schema parity and test coverage.
-  `to_domain` function L54-67 — `(r: UnifiedDeliveryOutbox) -> DeliveryOutbox` — schema parity and test coverage.
-  `enqueue_postgres` function L90-110 — `( &self, new: NewDeliveryOutbox, ) -> Result<DeliveryOutbox, ValidationError>` — schema parity and test coverage.
-  `enqueue_sqlite` function L113-133 — `( &self, new: NewDeliveryOutbox, ) -> Result<DeliveryOutbox, ValidationError>` — schema parity and test coverage.
-  `mark_delivered_postgres` function L149-173 — `(&self, id: i64) -> Result<usize, ValidationError>` — schema parity and test coverage.
-  `mark_delivered_sqlite` function L176-200 — `(&self, id: i64) -> Result<usize, ValidationError>` — schema parity and test coverage.
-  `mark_acked_postgres` function L222-245 — `(&self, id: i64) -> Result<usize, ValidationError>` — schema parity and test coverage.
-  `mark_acked_sqlite` function L248-271 — `(&self, id: i64) -> Result<usize, ValidationError>` — schema parity and test coverage.
-  `reset_to_pending_postgres` function L284-303 — `(&self, id: i64) -> Result<usize, ValidationError>` — schema parity and test coverage.
-  `reset_to_pending_sqlite` function L306-325 — `(&self, id: i64) -> Result<usize, ValidationError>` — schema parity and test coverage.
-  `reset_delivered_for_recipient_postgres` function L351-379 — `( &self, recipient: String, tenant_id: Option<String>, ) -> Result<usize, Valida...` — schema parity and test coverage.
-  `reset_delivered_for_recipient_sqlite` function L382-409 — `( &self, recipient: String, tenant_id: Option<String>, ) -> Result<usize, Valida...` — schema parity and test coverage.
-  `reassign_open_rows_postgres` function L443-471 — `( &self, from_recipient: String, to_recipient: String, ) -> Result<usize, Valida...` — schema parity and test coverage.
-  `reassign_open_rows_sqlite` function L474-502 — `( &self, from_recipient: String, to_recipient: String, ) -> Result<usize, Valida...` — schema parity and test coverage.
-  `list_open_for_recipient_postgres` function L523-546 — `( &self, recipient: String, limit: i64, ) -> Result<Vec<DeliveryOutbox>, Validat...` — schema parity and test coverage.
-  `list_open_for_recipient_sqlite` function L549-572 — `( &self, recipient: String, limit: i64, ) -> Result<Vec<DeliveryOutbox>, Validat...` — schema parity and test coverage.
-  `list_pending_postgres` function L586-607 — `( &self, limit: i64, ) -> Result<Vec<DeliveryOutbox>, ValidationError>` — schema parity and test coverage.
-  `list_pending_sqlite` function L610-631 — `( &self, limit: i64, ) -> Result<Vec<DeliveryOutbox>, ValidationError>` — schema parity and test coverage.
-  `list_stuck_postgres` function L648-671 — `( &self, cutoff: UniversalTimestamp, limit: i64, ) -> Result<Vec<DeliveryOutbox>...` — schema parity and test coverage.
-  `list_stuck_sqlite` function L674-697 — `( &self, cutoff: UniversalTimestamp, limit: i64, ) -> Result<Vec<DeliveryOutbox>...` — schema parity and test coverage.
-  `count_open_postgres` function L709-726 — `(&self) -> Result<i64, ValidationError>` — schema parity and test coverage.
-  `count_open_sqlite` function L729-746 — `(&self) -> Result<i64, ValidationError>` — schema parity and test coverage.
-  `build_insert` function L750-760 — `(new: NewDeliveryOutbox) -> NewUnifiedDeliveryOutbox` — Builds the insertable row for a new `pending` outbox entry.
-  `transition_result` function L765-780 — `( id: i64, from: &str, to: &str, affected: usize, ) -> Result<(), ValidationErro...` — Maps a compare-and-set affected-row count to a transition result: exactly
-  `tests` module L783-1154 — `-` — schema parity and test coverage.
-  `unique_dal` function L788-798 — `() -> DAL` — schema parity and test coverage.
-  `new_row` function L801-808 — `(recipient: &str) -> NewDeliveryOutbox` — schema parity and test coverage.
-  `test_enqueue_starts_pending` function L812-824 — `()` — schema parity and test coverage.
-  `test_reassign_open_rows_retargets_and_resets` function L828-876 — `()` — schema parity and test coverage.
-  `test_full_lifecycle_pending_delivered_acked` function L880-899 — `()` — schema parity and test coverage.
-  `test_mark_delivered_increments_attempts_and_stamps` function L903-921 — `()` — schema parity and test coverage.
-  `test_redelivery_resets_then_redelivers_incrementing_attempts` function L925-945 — `()` — schema parity and test coverage.
-  `test_ack_on_pending_succeeds_for_relay_recipient_race` function L949-970 — `()` — schema parity and test coverage.
-  `test_invalid_transition_rejected` function L974-1009 — `()` — schema parity and test coverage.
-  `test_list_open_for_recipient_isolates_and_orders` function L1013-1040 — `()` — schema parity and test coverage.
-  `test_reset_delivered_to_pending_isolates_by_recipient_and_tenant` function L1044-1110 — `()` — schema parity and test coverage.
-  `test_reset_delivered_to_pending_matches_null_tenant` function L1114-1153 — `()` — schema parity and test coverage.

#### crates/cloacina/src/dal/unified/execution_event.rs

- pub `ExecutionEventDAL` struct L72-74 — `{ dal: &'a DAL }` — Data access layer for execution event operations with runtime backend selection.
- pub `new` function L84-86 — `(dal: &'a DAL) -> Self` — Creates a new ExecutionEventDAL instance.
- pub `create` function L92-101 — `( &self, new_event: NewExecutionEvent, ) -> Result<ExecutionEvent, ValidationErr...` — Creates a new execution event record.
- pub `list_by_workflow` function L210-219 — `( &self, workflow_execution_id: UniversalUuid, ) -> Result<Vec<ExecutionEvent>, ...` — Gets all execution events for a specific workflow execution, ordered by sequence.
- pub `delete_older_than` function L465-474 — `( &self, cutoff: UniversalTimestamp, ) -> Result<usize, ValidationError>` — Deletes execution events older than the specified timestamp.
- pub `count_older_than` function L591-600 — `( &self, cutoff: UniversalTimestamp, ) -> Result<i64, ValidationError>` — Counts execution events older than the specified timestamp.
-  `build_event_outbox_row` function L40-65 — `( event_id: UniversalUuid, new_event: &NewExecutionEvent, now: UniversalTimestam...` — Substrate (CLOACI-I-0115 / T-0629): build the `delivery_outbox` row that
-  `create_postgres` function L104-150 — `( &self, new_event: NewExecutionEvent, ) -> Result<ExecutionEvent, ValidationErr...` — state transitions for debugging, compliance, and replay capability.
-  `create_sqlite` function L153-207 — `( &self, new_event: NewExecutionEvent, ) -> Result<ExecutionEvent, ValidationErr...` — state transitions for debugging, compliance, and replay capability.
-  `list_by_workflow_postgres` function L222-244 — `( &self, workflow_execution_id: UniversalUuid, ) -> Result<Vec<ExecutionEvent>, ...` — state transitions for debugging, compliance, and replay capability.
-  `list_by_workflow_sqlite` function L247-269 — `( &self, workflow_execution_id: UniversalUuid, ) -> Result<Vec<ExecutionEvent>, ...` — state transitions for debugging, compliance, and replay capability.
-  `list_by_task` function L272-281 — `( &self, task_execution_id: UniversalUuid, ) -> Result<Vec<ExecutionEvent>, Vali...` — Gets all execution events for a specific task execution, ordered by sequence.
-  `list_by_task_postgres` function L284-306 — `( &self, task_execution_id: UniversalUuid, ) -> Result<Vec<ExecutionEvent>, Vali...` — state transitions for debugging, compliance, and replay capability.
-  `list_by_task_sqlite` function L309-331 — `( &self, task_execution_id: UniversalUuid, ) -> Result<Vec<ExecutionEvent>, Vali...` — state transitions for debugging, compliance, and replay capability.
-  `list_by_type` function L334-344 — `( &self, event_type: ExecutionEventType, limit: i64, ) -> Result<Vec<ExecutionEv...` — Gets execution events by type for monitoring and analysis.
-  `list_by_type_postgres` function L347-372 — `( &self, event_type: ExecutionEventType, limit: i64, ) -> Result<Vec<ExecutionEv...` — state transitions for debugging, compliance, and replay capability.
-  `list_by_type_sqlite` function L375-400 — `( &self, event_type: ExecutionEventType, limit: i64, ) -> Result<Vec<ExecutionEv...` — state transitions for debugging, compliance, and replay capability.
-  `get_recent` function L403-412 — `( &self, limit: i64, ) -> Result<Vec<ExecutionEvent>, ValidationError>` — Gets recent execution events for monitoring purposes.
-  `get_recent_postgres` function L415-437 — `( &self, limit: i64, ) -> Result<Vec<ExecutionEvent>, ValidationError>` — state transitions for debugging, compliance, and replay capability.
-  `get_recent_sqlite` function L440-459 — `(&self, limit: i64) -> Result<Vec<ExecutionEvent>, ValidationError>` — state transitions for debugging, compliance, and replay capability.
-  `delete_older_than_postgres` function L477-499 — `( &self, cutoff: UniversalTimestamp, ) -> Result<usize, ValidationError>` — state transitions for debugging, compliance, and replay capability.
-  `delete_older_than_sqlite` function L502-524 — `( &self, cutoff: UniversalTimestamp, ) -> Result<usize, ValidationError>` — state transitions for debugging, compliance, and replay capability.
-  `count_by_workflow` function L527-536 — `( &self, workflow_execution_id: UniversalUuid, ) -> Result<i64, ValidationError>` — Counts total execution events for a workflow execution.
-  `count_by_workflow_postgres` function L539-561 — `( &self, workflow_execution_id: UniversalUuid, ) -> Result<i64, ValidationError>` — state transitions for debugging, compliance, and replay capability.
-  `count_by_workflow_sqlite` function L564-586 — `( &self, workflow_execution_id: UniversalUuid, ) -> Result<i64, ValidationError>` — state transitions for debugging, compliance, and replay capability.
-  `count_older_than_postgres` function L603-625 — `( &self, cutoff: UniversalTimestamp, ) -> Result<i64, ValidationError>` — state transitions for debugging, compliance, and replay capability.
-  `count_older_than_sqlite` function L628-650 — `( &self, cutoff: UniversalTimestamp, ) -> Result<i64, ValidationError>` — state transitions for debugging, compliance, and replay capability.
-  `substrate_producer_tests` module L654-756 — `-` — state transitions for debugging, compliance, and replay capability.
-  `unique_dal` function L659-669 — `() -> DAL` — state transitions for debugging, compliance, and replay capability.
-  `make_wf_exec` function L671-682 — `(dal: &DAL) -> UniversalUuid` — state transitions for debugging, compliance, and replay capability.
-  `event_create_also_enqueues_delivery_outbox_row` function L688-721 — `()` — Creating an execution event also enqueues a `delivery_outbox` row with
-  `two_events_produce_two_outbox_rows_for_same_recipient` function L726-755 — `()` — Two events for the same workflow produce two distinct outbox rows under
...
- pub `delivery_outbox` module L50 — `-` — ```
- pub `execution_event` module L51 — `-` — ```
- pub `models` module L52 — `-` — ```
- pub `reactor_subscriptions` module L53 — `-` — ```
- pub `recovery_event` module L54 — `-` — ```
- pub `schedule` module L55 — `-` — ```
- pub `schedule_execution` module L56 — `-` — ```
- pub `task_execution` module L57 — `-` — ```
- pub `task_execution_metadata` module L58 — `-` — ```
- pub `task_outbox` module L59 — `-` — ```
- pub `workflow_execution` module L60 — `-` — ```
- pub `workflow_packages` module L61 — `-` — ```
- pub `workflow_registry_storage` module L62 — `-` — ```
- pub `DAL` struct L97-100 — `{ database: Database }` — Helper macro for dispatching operations based on backend type.
- pub `new` function L112-114 — `(database: Database) -> Self` — Creates a new unified DAL instance.
- pub `backend` function L117-119 — `(&self) -> BackendType` — Returns the backend type for this DAL instance.
- pub `database` function L122-124 — `(&self) -> &Database` — Returns a reference to the underlying database.
- pub `pool` function L127-129 — `(&self) -> AnyPool` — Returns the connection pool.
- pub `api_keys` function L133-135 — `(&self) -> ApiKeyDAL<'_>` — Returns an API key DAL (Postgres only).
- pub `checkpoint` function L138-140 — `(&self) -> CheckpointDAL<'_>` — Returns a checkpoint DAL for computation graph state persistence.
- pub `context` function L143-145 — `(&self) -> ContextDAL<'_>` — Returns a context DAL for context operations.
- pub `workflow_execution` function L148-150 — `(&self) -> WorkflowExecutionDAL<'_>` — Returns a workflow execution DAL for workflow execution operations.
- pub `task_execution` function L153-155 — `(&self) -> TaskExecutionDAL<'_>` — Returns a task execution DAL for task operations.
- pub `task_execution_metadata` function L158-160 — `(&self) -> TaskExecutionMetadataDAL<'_>` — Returns a task execution metadata DAL for metadata operations.
- pub `task_outbox` function L163-165 — `(&self) -> TaskOutboxDAL<'_>` — Returns a task outbox DAL for work distribution operations.
- pub `delivery_outbox` function L169-171 — `(&self) -> DeliveryOutboxDAL<'_>` — Returns a delivery outbox DAL for the interservice communication
- pub `recovery_event` function L174-176 — `(&self) -> RecoveryEventDAL<'_>` — Returns a recovery event DAL for recovery operations.
- pub `execution_event` function L179-181 — `(&self) -> ExecutionEventDAL<'_>` — Returns an execution event DAL for execution event operations.
- pub `schedule` function L184-186 — `(&self) -> ScheduleDAL<'_>` — Returns a unified schedule DAL for schedule operations.
- pub `schedule_execution` function L189-191 — `(&self) -> ScheduleExecutionDAL<'_>` — Returns a unified schedule execution DAL for schedule execution operations.
- pub `workflow_packages` function L194-196 — `(&self) -> WorkflowPackagesDAL<'_>` — Returns a workflow packages DAL for package operations.
- pub `reactor_subscriptions` function L200-202 — `(&self) -> ReactorSubscriptionsDAL<'_>` — Returns a reactor subscriptions DAL for reactor-triggered workflow
- pub `workflow_registry` function L214-220 — `( &self, storage: S, ) -> crate::registry::workflow_registry::WorkflowRegistryIm...` — Creates a workflow registry implementation with the given storage backend.
- pub `try_workflow_registry` function L233-244 — `( &self, storage: S, ) -> Result< crate::registry::workflow_registry::WorkflowRe...` — Creates a workflow registry implementation with the given storage backend.
-  `DAL` type L102-245 — `= DAL` — ```
...
- pub `UnifiedDeliveryOutbox` struct L272-283 — `{ id: i64, recipient: String, kind: String, tenant_id: Option<String>, payload: ...` — Unified delivery-outbox row: durable, ack-tracked, recipient-addressed
- pub `NewUnifiedDeliveryOutbox` struct L287-295 — `{ recipient: String, kind: String, tenant_id: Option<String>, payload: Universal...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `UnifiedSchedule` struct L303-321 — `{ id: UniversalUuid, schedule_type: String, workflow_name: String, enabled: Univ...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `NewUnifiedSchedule` struct L325-341 — `{ id: UniversalUuid, schedule_type: String, workflow_name: String, enabled: Univ...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `UnifiedScheduleExecution` struct L349-360 — `{ id: UniversalUuid, schedule_id: UniversalUuid, workflow_execution_id: Option<U...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `NewUnifiedScheduleExecution` struct L364-374 — `{ id: UniversalUuid, schedule_id: UniversalUuid, workflow_execution_id: Option<U...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `UnifiedWorkflowRegistryEntry` struct L382-386 — `{ id: UniversalUuid, created_at: UniversalTimestamp, data: UniversalBinary }` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `NewUnifiedWorkflowRegistryEntry` struct L390-394 — `{ id: UniversalUuid, created_at: UniversalTimestamp, data: UniversalBinary }` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `UnifiedWorkflowPackage` struct L402-421 — `{ id: UniversalUuid, registry_id: UniversalUuid, package_name: String, version: ...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `NewUnifiedWorkflowPackage` struct L425-444 — `{ id: UniversalUuid, registry_id: UniversalUuid, package_name: String, version: ...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `UnifiedSigningKey` struct L452-461 — `{ id: UniversalUuid, org_id: UniversalUuid, key_name: String, encrypted_private_...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `NewUnifiedSigningKey` struct L465-473 — `{ id: UniversalUuid, org_id: UniversalUuid, key_name: String, encrypted_private_...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `UnifiedTrustedKey` struct L481-489 — `{ id: UniversalUuid, org_id: UniversalUuid, key_fingerprint: String, public_key:...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `NewUnifiedTrustedKey` struct L493-500 — `{ id: UniversalUuid, org_id: UniversalUuid, key_fingerprint: String, public_key:...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `UnifiedKeyTrustAcl` struct L508-514 — `{ id: UniversalUuid, parent_org_id: UniversalUuid, child_org_id: UniversalUuid, ...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `NewUnifiedKeyTrustAcl` struct L518-523 — `{ id: UniversalUuid, parent_org_id: UniversalUuid, child_org_id: UniversalUuid, ...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `UnifiedPackageSignature` struct L531-538 — `{ id: UniversalUuid, package_hash: String, key_fingerprint: String, signature: U...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `NewUnifiedPackageSignature` struct L542-549 — `{ id: UniversalUuid, package_hash: String, key_fingerprint: String, signature: U...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `UnifiedAccumulatorCheckpoint` struct L813-820 — `{ id: UniversalUuid, graph_name: String, accumulator_name: String, checkpoint_da...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `NewUnifiedAccumulatorCheckpoint` struct L824-831 — `{ id: UniversalUuid, graph_name: String, accumulator_name: String, checkpoint_da...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `UnifiedAccumulatorBoundary` struct L835-843 — `{ id: UniversalUuid, graph_name: String, accumulator_name: String, boundary_data...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `NewUnifiedAccumulatorBoundary` struct L847-855 — `{ id: UniversalUuid, graph_name: String, accumulator_name: String, boundary_data...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `UnifiedReactorState` struct L859-867 — `{ id: UniversalUuid, graph_name: String, cache_data: UniversalBinary, dirty_flag...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `NewUnifiedReactorState` struct L871-879 — `{ id: UniversalUuid, graph_name: String, cache_data: UniversalBinary, dirty_flag...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `UnifiedStateAccumulatorBuffer` struct L883-891 — `{ id: UniversalUuid, graph_name: String, accumulator_name: String, buffer_data: ...` — SQL types that work with both PostgreSQL and SQLite backends.
- pub `NewUnifiedStateAccumulatorBuffer` struct L895-903 — `{ id: UniversalUuid, graph_name: String, accumulator_name: String, buffer_data: ...` — SQL types that work with both PostgreSQL and SQLite backends.
...
-  `WorkflowExecutionRecord` type L582-601 — `= WorkflowExecutionRecord` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L583-600 — `(u: UnifiedWorkflowExecution) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
-  `TaskExecution` type L603-628 — `= TaskExecution` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L604-627 — `(u: UnifiedTaskExecution) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
-  `TaskExecutionMetadata` type L630-642 — `= TaskExecutionMetadata` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L631-641 — `(u: UnifiedTaskExecutionMetadata) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
-  `RecoveryEvent` type L644-657 — `= RecoveryEvent` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L645-656 — `(u: UnifiedRecoveryEvent) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
-  `ExecutionEvent` type L659-675 — `= ExecutionEvent` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L660-674 — `(u: UnifiedExecutionEvent) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
-  `WorkflowRegistryEntry` type L677-685 — `= WorkflowRegistryEntry` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L678-684 — `(u: UnifiedWorkflowRegistryEntry) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
-  `WorkflowPackage` type L687-710 — `= WorkflowPackage` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L688-709 — `(u: UnifiedWorkflowPackage) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
-  `SigningKey` type L712-725 — `= SigningKey` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L713-724 — `(u: UnifiedSigningKey) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
-  `TrustedKey` type L727-739 — `= TrustedKey` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L728-738 — `(u: UnifiedTrustedKey) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
-  `KeyTrustAcl` type L741-751 — `= KeyTrustAcl` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L742-750 — `(u: UnifiedKeyTrustAcl) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
-  `PackageSignature` type L753-764 — `= PackageSignature` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L754-763 — `(u: UnifiedPackageSignature) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
-  `Schedule` type L766-788 — `= Schedule` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L767-787 — `(u: UnifiedSchedule) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
-  `ScheduleExecution` type L790-805 — `= ScheduleExecution` — SQL types that work with both PostgreSQL and SQLite backends.
-  `from` function L791-804 — `(u: UnifiedScheduleExecution) -> Self` — SQL types that work with both PostgreSQL and SQLite backends.
...
- pub `get_compiled_data_by_content_hash` function L571-582 — `( &self, content_hash: &str, ) -> Result<Option<Vec<u8>>, RegistryError>` — Look up a compiled package by its `content_hash` digest and return
- pub `get_active_content_hash_for_package` function L644-658 — `( &self, package_name: &str, tenant_id: Option<&str>, ) -> Result<Option<String>...` — Resolve the **active** content-hash (digest) for a package name within a
...
-  `get_compiled_data_by_content_hash_postgres` function L585-608 — `( &self, content_hash: &str, ) -> Result<Option<Vec<u8>>, RegistryError>` — at runtime based on the database connection type.
-  `get_compiled_data_by_content_hash_sqlite` function L611-634 — `( &self, content_hash: &str, ) -> Result<Option<Vec<u8>>, RegistryError>` — at runtime based on the database connection type.
-  `get_active_content_hash_postgres` function L661-696 — `( &self, package_name: String, tenant_id: Option<String>, ) -> Result<Option<Str...` — at runtime based on the database connection type.
-  `get_active_content_hash_sqlite` function L699-734 — `( &self, package_name: String, tenant_id: Option<String>, ) -> Result<Option<Str...` — at runtime based on the database connection type.
-  `tests` module L738-990 — `-` — at runtime based on the database connection type.
-  `unique_dal` function L744-754 — `() -> DAL` — at runtime based on the database connection type.
-  `sample_metadata` function L757-776 — `(name: &str, version: &str) -> PackageMetadata` — at runtime based on the database connection type.
-  `test_store_and_get_package_metadata` function L780-809 — `()` — at runtime based on the database connection type.
-  `test_get_package_metadata_not_found` function L813-822 — `()` — at runtime based on the database connection type.
-  `test_get_package_metadata_by_id` function L826-850 — `()` — at runtime based on the database connection type.
-  `test_get_package_metadata_by_id_not_found` function L854-863 — `()` — at runtime based on the database connection type.
-  `test_list_all_packages` function L867-899 — `()` — at runtime based on the database connection type.
-  `test_delete_package_metadata` function L903-939 — `()` — at runtime based on the database connection type.
-  `test_delete_package_metadata_by_id` function L943-972 — `()` — at runtime based on the database connection type.
-  `test_delete_nonexistent_does_not_error` function L976-989 — `()` — at runtime based on the database connection type.
...
- pub `unified` module L1177-1179 — `-`
- pub `postgres` module L1185-1187 — `-`
- pub `sqlite` module L1190-1192 — `-`
-  `unified_schema` module L25-462 — `-`
-  `postgres_schema` module L471-860 — `-`
-  `sqlite_schema` module L863-1172 — `-`
...
- pub `envelope` module L38 — `-` — Delivery relay for the interservice communication substrate
- pub `sweeper` module L39 — `-` — unit-tested on SQLite.
- pub `DeliveryError` enum L58-61 — `Sink` — Errors a [`DeliverySink`] can report.
- pub `DeliveryOutcome` enum L65-73 — `Delivered | NoRoute` — Outcome of handing a row to a sink.
- pub `DeliverySink` interface L80-82 — `{ fn deliver() }` — Transport that actually delivers an outbox row to its addressed recipient.
- pub `WakeHandle` struct L86-88 — `{ notify: Arc<Notify> }` — A cloneable handle producers (and the LISTEN task) use to wake the relay.
- pub `wake` function L95-97 — `(&self)` — Wake the relay to drain.
- pub `DeliveryRelay` struct L101-106 — `{ dal: DAL, sink: Arc<dyn DeliverySink>, notify: Arc<Notify>, drain_batch: i64 }` — Drains the delivery outbox on demand and pushes rows to a [`DeliverySink`].
- pub `new` function L110-117 — `(dal: DAL, sink: Arc<dyn DeliverySink>) -> Self` — Creates a relay over the given DAL and sink.
- pub `with_drain_batch` function L120-123 — `(mut self, batch: i64) -> Self` — Overrides the per-wake drain batch size (defaults to [`DEFAULT_DRAIN_BATCH`]).
- pub `wake_handle` function L126-130 — `(&self) -> WakeHandle` — Returns a wake handle for producers / the LISTEN task.
- pub `drain_once` function L135-176 — `(&self) -> Result<usize, crate::error::ValidationError>` — Drains one batch of `pending` rows: deliver each via the sink, mark the
- pub `run` function L181-201 — `(self, mut shutdown: watch::Receiver<bool>)` — Runs the relay until `shutdown` flips to `true`.
- pub `run_pg_listener` function L213-238 — `( conn_str: String, channel: String, wake: WakeHandle, mut shutdown: watch::Rece...` — Runs a Postgres `LISTEN` loop on `channel`, waking `wake` on every
-  `DEFAULT_DRAIN_BATCH` variable L53 — `: i64` — Default number of rows drained per wake before yielding back to the wait.
-  `WakeHandle` type L90-98 — `= WakeHandle` — unit-tested on SQLite.
-  `DeliveryRelay` type L108-202 — `= DeliveryRelay` — unit-tested on SQLite.
-  `BACKOFF` variable L220 — `: Duration` — unit-tested on SQLite.
-  `listen_once` function L243-303 — `( conn_str: &str, channel: &str, wake: &WakeHandle, shutdown: &mut watch::Receiv...` — One LISTEN session.
-  `tests` module L306-432 — `-` — unit-tested on SQLite.
-  `unique_dal` function L313-323 — `() -> DAL` — unit-tested on SQLite.
-  `work` function L325-332 — `(recipient: &str) -> NewDeliveryOutbox` — unit-tested on SQLite.
-  `CollectingSink` struct L336-339 — `{ seen: Mutex<Vec<i64>>, outcome: DeliveryOutcome }` — A sink that records the ids it was asked to deliver, with a configurable
-  `CollectingSink` type L341-351 — `= CollectingSink` — unit-tested on SQLite.
-  `new` function L342-347 — `(outcome: DeliveryOutcome) -> Arc<Self>` — unit-tested on SQLite.
-  `seen` function L348-350 — `(&self) -> Vec<i64>` — unit-tested on SQLite.
-  `CollectingSink` type L354-359 — `impl DeliverySink for CollectingSink` — unit-tested on SQLite.
-  `deliver` function L355-358 — `(&self, row: &DeliveryOutbox) -> Result<DeliveryOutcome, DeliveryError>` — unit-tested on SQLite.
-  `test_drain_delivers_pending_and_marks_delivered` function L362-378 — `()` — unit-tested on SQLite.
-  `test_no_route_leaves_row_pending` function L381-398 — `()` — unit-tested on SQLite.
-  `test_in_process_wake_triggers_drain` function L401-431 — `()` — unit-tested on SQLite.

#### crates/cloacina/src/delivery/sweeper.rs

- pub `SweeperConfig` struct L57-65 — `{ sweep_interval: Duration, stuck_threshold: Duration, batch_limit: i64 }` — Tunables for the sweeper.
- pub `DeliverySweeper` struct L78-82 — `{ dal: DAL, wake: WakeHandle, config: SweeperConfig }` — Periodic backstop scan over the delivery outbox.
- pub `new` function L86-88 — `(dal: DAL, wake: WakeHandle) -> Self` — Construct with [`SweeperConfig::default`].
- pub `with_config` function L90-92 — `(dal: DAL, wake: WakeHandle, config: SweeperConfig) -> Self` — of what the former "anything ready?" poll loops produced.
- pub `run` function L95-116 — `(self, mut shutdown: watch::Receiver<bool>)` — Run until `shutdown` flips to `true` (or the sender drops).
- pub `sweep_once` function L121-170 — `(&self) -> Result<usize, ValidationError>` — One sweep pass.
-  `SweeperConfig` type L67-75 — `impl Default for SweeperConfig` — of what the former "anything ready?" poll loops produced.
-  `default` function L68-74 — `() -> Self` — of what the former "anything ready?" poll loops produced.
-  `DeliverySweeper` type L84-171 — `= DeliverySweeper` — of what the former "anything ready?" poll loops produced.
-  `tests` module L174-328 — `-` — of what the former "anything ready?" poll loops produced.
-  `unique_dal` function L182-192 — `() -> DAL` — of what the former "anything ready?" poll loops produced.
-  `work` function L194-201 — `(recipient: &str) -> NewDeliveryOutbox` — of what the former "anything ready?" poll loops produced.
-  `NullSink` struct L205 — `-` — Minimal sink so we can construct a real relay (and thus a real wake
-  `NullSink` type L207-214 — `impl DeliverySink for NullSink` — of what the former "anything ready?" poll loops produced.
-  `deliver` function L208-213 — `( &self, _row: &DeliveryOutbox, ) -> Result<DeliveryOutcome, crate::delivery::De...` — of what the former "anything ready?" poll loops produced.
-  `make_sweeper` function L216-228 — `(dal: DAL, threshold: Duration) -> DeliverySweeper` — of what the former "anything ready?" poll loops produced.
-  `sweep_resets_stuck_delivered_rows` function L231-249 — `()` — of what the former "anything ready?" poll loops produced.
-  `sweep_skips_fresh_rows` function L252-274 — `()` — of what the former "anything ready?" poll loops produced.
-  `second_sweep_is_idempotent` function L277-297 — `()` — of what the former "anything ready?" poll loops produced.
-  `concurrent_sweepers_are_race_safe` function L302-327 — `()` — Concurrent sweepers racing on the same stuck row: at most one wins the
...
- pub `DefaultDispatcher` struct L53-60 — `{ executors: RwLock<HashMap<String, Arc<dyn TaskExecutor>>>, default_executor_ke...` — Default dispatcher implementation.
- pub `new` function L66-72 — `(dal: DAL, default_executor: impl Into<String>) -> Self` — Creates a new DefaultDispatcher that dispatches every task to
- pub `with_defaults` function L76-78 — `(dal: DAL) -> Self` — Creates a dispatcher that sends every task to the `"default"` (thread)
- pub `default_executor_key` function L81-83 — `(&self) -> &str` — The executor key every task is dispatched to.
- pub `dal` function L86-88 — `(&self) -> &DAL` — Gets a reference to the DAL.
-  `DefaultDispatcher` type L62-137 — `= DefaultDispatcher` — dispatcher only know the one configured executor key.
-  `handle_result` function L93-136 — `( &self, event: &TaskReadyEvent, result: super::types::ExecutionResult, ) -> Res...` — Logs the execution result.
-  `DefaultDispatcher` type L140-195 — `impl Dispatcher for DefaultDispatcher` — dispatcher only know the one configured executor key.
-  `dispatch` function L141-169 — `(&self, event: TaskReadyEvent) -> Result<(), DispatchError>` — dispatcher only know the one configured executor key.
-  `register_executor` function L171-179 — `(&self, key: &str, executor: Arc<dyn TaskExecutor>)` — dispatcher only know the one configured executor key.
-  `has_capacity` function L181-184 — `(&self) -> bool` — dispatcher only know the one configured executor key.
-  `resolve_executor_key` function L186-190 — `(&self, _task_name: &str) -> String` — dispatcher only know the one configured executor key.
-  `has_executor` function L192-194 — `(&self, key: &str) -> bool` — dispatcher only know the one configured executor key.
-  `tests` module L198-363 — `-` — dispatcher only know the one configured executor key.
-  `MockExecutor` struct L206-210 — `{ name: String, has_capacity: AtomicBool, execute_count: AtomicUsize }` — Mock executor for testing
-  `MockExecutor` type L212-225 — `= MockExecutor` — dispatcher only know the one configured executor key.
-  `new` function L213-219 — `(name: &str) -> Self` — dispatcher only know the one configured executor key.
-  `execution_count` function L222-224 — `(&self) -> usize` — dispatcher only know the one configured executor key.
-  `MockExecutor` type L228-254 — `impl TaskExecutor for MockExecutor` — dispatcher only know the one configured executor key.
-  `execute` function L229-235 — `(&self, event: TaskReadyEvent) -> Result<ExecutionResult, DispatchError>` — dispatcher only know the one configured executor key.
-  `has_capacity` function L237-239 — `(&self) -> bool` — dispatcher only know the one configured executor key.
-  `metrics` function L241-249 — `(&self) -> ExecutorMetrics` — dispatcher only know the one configured executor key.
-  `name` function L251-253 — `(&self) -> &str` — dispatcher only know the one configured executor key.
-  `create_test_event` function L257-264 — `(task_name: &str) -> TaskReadyEvent` — dispatcher only know the one configured executor key.
-  `test_register_executor` function L267-271 — `()` — dispatcher only know the one configured executor key.
-  `test_mock_executor_has_capacity` function L274-280 — `()` — dispatcher only know the one configured executor key.
-  `test_mock_executor_metrics` function L283-288 — `()` — dispatcher only know the one configured executor key.
-  `test_mock_executor_name` function L291-294 — `()` — dispatcher only know the one configured executor key.
-  `test_mock_executor_execute_increments_count` function L297-308 — `()` — dispatcher only know the one configured executor key.
-  `test_task_ready_event_creation` function L311-315 — `()` — dispatcher only know the one configured executor key.
-  `test_execution_result_success` function L318-324 — `()` — dispatcher only know the one configured executor key.
-  `test_execution_result_failure` function L327-332 — `()` — dispatcher only know the one configured executor key.
-  `test_execution_result_retry` function L335-340 — `()` — dispatcher only know the one configured executor key.
-  `test_executor_metrics_available_capacity` function L343-352 — `()` — dispatcher only know the one configured executor key.
-  `test_executor_metrics_at_capacity` function L355-362 — `()` — dispatcher only know the one configured executor key.

#### crates/cloacina/src/dispatcher/mod.rs

- pub `default` module L56 — `-` — # Dispatcher Layer for Executor Decoupling
- pub `traits` module L57 — `-` — ```
- pub `types` module L58 — `-` — ```

#### crates/cloacina/src/dispatcher/traits.rs

- pub `Dispatcher` interface L61-106 — `{ fn dispatch(), fn register_executor(), fn has_capacity(), fn resolve_executor_...` — Dispatcher hands task-ready events to the configured executor.
- pub `TaskExecutor` interface L144-177 — `{ fn execute(), fn has_capacity(), fn metrics(), fn name() }` — Executor receives task-ready events and executes them.
...
- pub `DispatchError` enum L162-190 — `ExecutorNotFound | ExecutionFailed | DatabaseError | ContextError | ValidationEr...` — Errors that can occur during dispatch operations.
...
#### crates/cloacina/src/executor/context_builder.rs

- pub `TaskContextBuilder` struct L42-44 — `{ dal: DAL }` — Builds a task's input context by loading + merging its dependency contexts
- pub `new` function L47-49 — `(dal: DAL) -> Self` — from the same server-side `Runtime`).
- pub `build` function L58-177 — `( &self, claimed_task: &ClaimedTask, dependencies: &[TaskNamespace], ) -> Result...` — Build the execution context for `claimed_task` given its `dependencies`.
- pub `merge_context_values` function L181-213 — `( existing: &serde_json::Value, new: &serde_json::Value, ) -> serde_json::Value` — Smart-merge two context values: arrays concat+dedup, objects merge
-  `TaskContextBuilder` type L46-214 — `= TaskContextBuilder` — from the same server-side `Runtime`).
-  `tests` module L217-247 — `-` — from the same server-side `Runtime`).
-  `merge_primitives_latest_wins` function L221-225 — `()` — from the same server-side `Runtime`).
-  `merge_arrays_dedup_concat` function L228-234 — `()` — from the same server-side `Runtime`).
-  `merge_objects_recursive` function L237-246 — `()` — from the same server-side `Runtime`).

#### crates/cloacina/src/executor/mod.rs

- pub `context_builder` module L47 — `-` — # Task Executor
- pub `result_handler` module L48 — `-` — All components are thread-safe and can be used in concurrent environments.
- pub `slot_token` module L49 — `-` — All components are thread-safe and can be used in concurrent environments.
- pub `task_handle` module L50 — `-` — All components are thread-safe and can be used in concurrent environments.
- pub `thread_task_executor` module L51 — `-` — All components are thread-safe and can be used in concurrent environments.
- pub `types` module L52 — `-` — All components are thread-safe and can be used in concurrent environments.
- pub `workflow_executor` module L53 — `-` — All components are thread-safe and can be used in concurrent environments.

#### crates/cloacina/src/executor/result_handler.rs

- pub `TaskResultHandler` struct L49-58 — `{ dal: DAL, total_executed: Arc<AtomicU64>, total_failed: Arc<AtomicU64>, runner...` — Post-execution handler — applies all state writes / retry decisions /
- pub `new` function L61-73 — `( dal: DAL, total_executed: Arc<AtomicU64>, total_failed: Arc<AtomicU64>, runner...` — retry policy + elapsed duration, and applies all persistent-state effects.
- pub `handle_outcome` function L85-150 — `( &self, event: &TaskReadyEvent, claimed_task: &ClaimedTask, outcome: Result<Con...` — Apply the post-execution state machine for one task and return the
- pub `is_transient_error` function L280-296 — `(&self, error: &ExecutorError) -> bool` — Classifies whether an error looks transient enough to retry under
-  `TaskResultHandler` type L60-326 — `= TaskResultHandler` — retry policy + elapsed duration, and applies all persistent-state effects.
-  `complete_task_transaction` function L160-196 — `( &self, claimed_task: &ClaimedTask, context: Context<serde_json::Value>, ) -> R...` — Completes a task by saving its context and then marking it completed.
-  `save_task_context` function L200-228 — `( &self, claimed_task: &ClaimedTask, context: Context<serde_json::Value>, ) -> R...` — Saves a task's produced context to the contexts table and links it via
-  `should_retry_task` function L232-274 — `( &self, claimed_task: &ClaimedTask, error: &ExecutorError, retry_policy: &Retry...` — Determines if a failed task should be retried, considering max-attempts,
-  `schedule_task_retry` function L300-325 — `( &self, claimed_task: &ClaimedTask, retry_policy: &RetryPolicy, ) -> Result<(),...` — Schedules a task for retry via the DAL, computing the delay from the
-  `is_transient_tests` module L329-403 — `-` — retry policy + elapsed duration, and applies all persistent-state effects.
-  `handler` function L337-346 — `() -> TaskResultHandler` — retry policy + elapsed duration, and applies all persistent-state effects.
-  `test_is_transient_timeout` function L349-351 — `()` — retry policy + elapsed duration, and applies all persistent-state effects.
-  `test_is_transient_task_not_found` function L354-356 — `()` — retry policy + elapsed duration, and applies all persistent-state effects.
-  `test_is_transient_connection_pool` function L359-362 — `()` — retry policy + elapsed duration, and applies all persistent-state effects.
-  `test_is_transient_task_execution_with_timeout_msg` function L365-372 — `()` — retry policy + elapsed duration, and applies all persistent-state effects.
-  `test_is_transient_task_execution_permanent` function L375-382 — `()` — retry policy + elapsed duration, and applies all persistent-state effects.
-  `test_is_transient_task_execution_network` function L385-392 — `()` — retry policy + elapsed duration, and applies all persistent-state effects.
-  `test_is_transient_task_execution_unavailable` function L395-402 — `()` — retry policy + elapsed duration, and applies all persistent-state effects.
...
- pub `ThreadTaskExecutor` struct L98-123 — `{ database: Database, dal: DAL, task_registry: Arc<TaskRegistry>, runtime: Arc<R...` — ThreadTaskExecutor is a thread-based implementation of task execution.
- pub `new` function L135-141 — `( database: Database, task_registry: Arc<TaskRegistry>, config: ExecutorConfig, ...` — Creates a new ThreadTaskExecutor instance.
- pub `with_runtime_and_registry` function L144-181 — `( database: Database, task_registry: Arc<TaskRegistry>, runtime: Arc<Runtime>, c...` — Creates a new ThreadTaskExecutor with a specific runtime.
- pub `with_runtime` function L184-187 — `(mut self, runtime: Arc<Runtime>) -> Self` — Sets the runtime for this executor, replacing the default.
- pub `semaphore` function L193-195 — `(&self) -> &Arc<Semaphore>` — Returns a reference to the concurrency semaphore.
-  `failure_reason` function L59-80 — `(err: &ExecutorError) -> &'static str` — Bounded reason value for `cloacina_tasks_total{status="failed", reason=...}`.
-  `ThreadTaskExecutor` type L125-308 — `= ThreadTaskExecutor` — to the executor based on routing rules.
-  `build_task_context` function L205-216 — `( &self, claimed_task: &ClaimedTask, dependencies: &[crate::task::TaskNamespace]...` — Builds the execution context for a task by loading its dependencies.
-  `merge_context_values` function L236-241 — `( existing: &serde_json::Value, new: &serde_json::Value, ) -> serde_json::Value` — Merges two context values using smart merging strategy.
-  `execute_with_timeout` function L251-260 — `( &self, task: &dyn Task, context: Context<serde_json::Value>, ) -> Result<Conte...` — Executes a task with timeout protection.
...
### crates/cloacina/src/fleet

> *Semantic summary to be generated by AI agent.*

#### crates/cloacina/src/fleet/mod.rs

- pub `protocol` module L25 — `-` — Execution-agent fleet protocol (CLOACI-I-0114).

#### crates/cloacina/src/fleet/protocol.rs

- pub `AGENT_PROTOCOL_VERSION` variable L53 — `: u32` — Wire-protocol version for the agent fleet.
- pub `WORK_PACKET_KIND` variable L57 — `: &str` — Substrate envelope `kind` value used when a `ServerMessage::Push` carries
- pub `AGENT_RECIPIENT_PREFIX` variable L61 — `: &str` — Substrate recipient prefix for agent connections.
- pub `DEFAULT_HEARTBEAT_INTERVAL_SECONDS` variable L65 — `: u32` — Default suggested heartbeat interval the server returns to a registering
- pub `host_target_triple` function L73-75 — `() -> String` — Best-effort host target triple.
- pub `AgentRegisterRequest` struct L82-98 — `{ protocol_version: u32, agent_id: Option<String>, max_concurrency: u32, target_...` — homogeneous; this is the guard that makes a misconfiguration loud.
- pub `AgentRegisterResponse` struct L101-111 — `{ protocol_version: u32, agent_id: String, heartbeat_interval_seconds: u32 }` — homogeneous; this is the guard that makes a misconfiguration loud.
- pub `AgentHeartbeatRequest` struct L118-126 — `{ protocol_version: u32, agent_id: String, in_flight: u32, available_capacity: u...` — homogeneous; this is the guard that makes a misconfiguration loud.
- pub `AgentHeartbeatResponse` struct L129-131 — `{ protocol_version: u32 }` — homogeneous; this is the guard that makes a misconfiguration loud.
- pub `WorkPacket` struct L144-163 — `{ protocol_version: u32, task_execution_id: String, workflow_execution_id: Strin...` — Fully self-contained work packet — everything a DB-less agent needs to
- pub `ArtifactRef` struct L167-177 — `{ digest: String, fetch_url: String, build_target_triple: String }` — Reference to a workflow artifact (cdylib) the agent must fetch + load.
- pub `AgentResultRequest` struct L184-193 — `{ protocol_version: u32, agent_id: String, task_execution_id: String, attempt: i...` — homogeneous; this is the guard that makes a misconfiguration loud.
- pub `AgentResultResponse` struct L196-198 — `{ protocol_version: u32 }` — homogeneous; this is the guard that makes a misconfiguration loud.
- pub `AgentOutcome` enum L207-221 — `Success | Failure | Refused` — Outcome of one work packet as reported by the agent.
- pub `FailureClassification` enum L228-238 — `TaskError | Transient | Validation | Timeout` — Bounded classification of a task-level failure so the server's retry
- pub `RefusalReason` enum L244-255 — `TargetTripleMismatch | ArtifactFetchFailed | RuntimeLoadFailed | Shutdown | Tena...` — Why the agent refused to even run the packet.
-  `tests` module L258-342 — `-` — homogeneous; this is the guard that makes a misconfiguration loud.
-  `work_packet_round_trips_as_json` function L262-283 — `()` — homogeneous; this is the guard that makes a misconfiguration loud.
-  `outcome_variants_round_trip_with_snake_case_tags` function L286-325 — `()` — homogeneous; this is the guard that makes a misconfiguration loud.
-  `agent_recipient_prefix_is_stable` function L328-333 — `()` — homogeneous; this is the guard that makes a misconfiguration loud.
-  `register_request_agent_id_defaults_to_none` function L336-341 — `()` — homogeneous; this is the guard that makes a misconfiguration loud.
...
#### crates/cloacina/src/models/delivery_outbox.rs

- pub `DeliveryState` enum L37-41 — `Pending | Delivered | Acked` — Delivery lifecycle of an outbox row.
- pub `as_str` function L45-51 — `(&self) -> &'static str` — String form persisted in the `delivery_state` column.
- pub `from_db` function L54-61 — `(s: &str) -> Option<DeliveryState>` — Parse from the persisted string form.
- pub `can_transition_to` function L70-76 — `(self, next: DeliveryState) -> bool` — Whether `self → next` is a permitted transition.
- pub `DeliveryOutbox` struct L81-102 — `{ id: i64, recipient: String, kind: String, tenant_id: Option<String>, payload: ...` — A delivery-outbox row (domain type).
- pub `state` function L106-108 — `(&self) -> Option<DeliveryState>` — Typed view of the persisted `delivery_state` string.
- pub `NewDeliveryOutbox` struct L116-121 — `{ recipient: String, kind: String, tenant_id: Option<String>, payload: Vec<u8> }` — Structure for enqueuing a new delivery-outbox row.
-  `DeliveryState` type L43-77 — `= DeliveryState` — substrate is Postgres-only at runtime.
-  `DeliveryOutbox` type L104-109 — `= DeliveryOutbox` — substrate is Postgres-only at runtime.
...
- pub `delivery_outbox` module L72 — `-` — - Keep model definitions in sync with database schema migrations
- pub `execution_event` module L73 — `-` — - Keep model definitions in sync with database schema migrations
...
- pub `workflow_execution` module L79 — `-` — - Keep model definitions in sync with database schema migrations
- pub `workflow_packages` module L80 — `-` — - Keep model definitions in sync with database schema migrations
- pub `workflow_registry` module L81 — `-` — - Keep model definitions in sync with database schema migrations
- pub `key_trust_acl` module L84 — `-` — - Keep model definitions in sync with database schema migrations
- pub `package_signature` module L85 — `-` — - Keep model definitions in sync with database schema migrations
- pub `signing_key` module L86 — `-` — - Keep model definitions in sync with database schema migrations
- pub `trusted_key` module L87 — `-` — - Keep model definitions in sync with database schema migrations
...
- pub `DefaultRunnerConfig` struct L68-109 — `{ max_concurrent_tasks: usize, scheduler_poll_interval: Duration, task_timeout: ...` — Configuration for the default runner
- pub `builder` function L113-115 — `() -> DefaultRunnerConfigBuilder` — Creates a new configuration builder with default values.
- pub `max_concurrent_tasks` function L118-120 — `(&self) -> usize` — Maximum number of concurrent task executions allowed.
- pub `scheduler_poll_interval` function L123-125 — `(&self) -> Duration` — How often the scheduler checks for ready tasks.
- pub `task_timeout` function L128-130 — `(&self) -> Duration` — Maximum time allowed for a single task to execute.
- pub `workflow_timeout` function L133-135 — `(&self) -> Option<Duration>` — Optional maximum time for an entire workflow execution.
- pub `db_pool_size` function L138-140 — `(&self) -> u32` — Number of database connections in the pool.
- pub `enable_recovery` function L143-145 — `(&self) -> bool` — Whether automatic recovery is enabled.
- pub `enable_cron_scheduling` function L148-150 — `(&self) -> bool` — Whether cron scheduling is enabled.
- pub `cron_poll_interval` function L153-155 — `(&self) -> Duration` — Poll interval for cron schedules.
- pub `cron_max_catchup_executions` function L158-160 — `(&self) -> usize` — Maximum catchup executions for missed cron runs.
- pub `cron_enable_recovery` function L163-165 — `(&self) -> bool` — Whether cron recovery is enabled.
- pub `cron_recovery_interval` function L168-170 — `(&self) -> Duration` — How often to check for lost cron executions.
- pub `cron_lost_threshold_minutes` function L173-175 — `(&self) -> i32` — Minutes before an execution is considered lost.
- pub `cron_max_recovery_age` function L178-180 — `(&self) -> Duration` — Maximum age of executions to recover.
- pub `cron_max_recovery_attempts` function L183-185 — `(&self) -> usize` — Maximum recovery attempts per execution.
- pub `enable_trigger_scheduling` function L188-190 — `(&self) -> bool` — Whether trigger scheduling is enabled.
- pub `trigger_base_poll_interval` function L193-195 — `(&self) -> Duration` — Base poll interval for trigger readiness checks.
- pub `trigger_poll_timeout` function L198-200 — `(&self) -> Duration` — Timeout for trigger poll operations.
- pub `enable_registry_reconciler` function L203-205 — `(&self) -> bool` — Whether the registry reconciler is enabled.
- pub `registry_reconcile_interval` function L208-210 — `(&self) -> Duration` — How often to run registry reconciliation.
- pub `registry_enable_startup_reconciliation` function L213-215 — `(&self) -> bool` — Whether startup reconciliation is enabled.
- pub `registry_storage_path` function L218-220 — `(&self) -> Option<&std::path::Path>` — Path for registry storage (filesystem backend).
- pub `require_signatures` function L224-226 — `(&self) -> bool` — CLOACI-T-0571: when true, the reconciler refuses to load packages
- pub `verification_org_id` function L230-232 — `(&self) -> Option<crate::UniversalUuid>` — Trusted org UUID forwarded to the reconciler for audit logging
- pub `registry_storage_backend` function L235-237 — `(&self) -> &str` — Registry storage backend type.
- pub `enable_claiming` function L240-242 — `(&self) -> bool` — Whether task claiming is enabled for horizontal scaling.
- pub `heartbeat_interval` function L245-247 — `(&self) -> Duration` — Heartbeat interval for claimed tasks.
- pub `stale_claim_sweep_interval` function L250-252 — `(&self) -> Duration` — Interval for stale claim sweep (only when claiming is enabled).
- pub `stale_claim_threshold` function L255-257 — `(&self) -> Duration` — How old a heartbeat must be to consider a claim stale.
- pub `runner_id` function L260-262 — `(&self) -> Option<&str>` — Optional runner identifier for logging.
- pub `runner_name` function L265-267 — `(&self) -> Option<&str>` — Optional runner name for logging.
- pub `default_executor` function L270-272 — `(&self) -> &str` — Executor key every task is dispatched to.
- pub `DefaultRunnerConfigBuilder` struct L286-288 — `{ config: DefaultRunnerConfig }` — Builder for [`DefaultRunnerConfig`].
- pub `max_concurrent_tasks` function L332-335 — `(mut self, value: usize) -> Self` — Sets the maximum number of concurrent task executions.
- pub `scheduler_poll_interval` function L338-341 — `(mut self, value: Duration) -> Self` — Sets the scheduler poll interval.
- pub `task_timeout` function L344-347 — `(mut self, value: Duration) -> Self` — Sets the task timeout.
- pub `workflow_timeout` function L350-353 — `(mut self, value: Option<Duration>) -> Self` — Sets the workflow timeout.
- pub `db_pool_size` function L356-359 — `(mut self, value: u32) -> Self` — Sets the database pool size.
- pub `enable_recovery` function L362-365 — `(mut self, value: bool) -> Self` — Enables or disables automatic recovery.
- pub `enable_cron_scheduling` function L368-371 — `(mut self, value: bool) -> Self` — Enables or disables cron scheduling.
- pub `cron_poll_interval` function L374-377 — `(mut self, value: Duration) -> Self` — Sets the cron poll interval.
- pub `cron_max_catchup_executions` function L380-383 — `(mut self, value: usize) -> Self` — Sets the maximum catchup executions for cron.
- pub `cron_enable_recovery` function L386-389 — `(mut self, value: bool) -> Self` — Enables or disables cron recovery.
- pub `cron_recovery_interval` function L392-395 — `(mut self, value: Duration) -> Self` — Sets the cron recovery interval.
- pub `cron_lost_threshold_minutes` function L398-401 — `(mut self, value: i32) -> Self` — Sets the cron lost threshold in minutes.
- pub `cron_max_recovery_age` function L404-407 — `(mut self, value: Duration) -> Self` — Sets the maximum cron recovery age.
- pub `cron_max_recovery_attempts` function L410-413 — `(mut self, value: usize) -> Self` — Sets the maximum cron recovery attempts.
- pub `enable_trigger_scheduling` function L416-419 — `(mut self, value: bool) -> Self` — Enables or disables trigger scheduling.
- pub `trigger_base_poll_interval` function L422-425 — `(mut self, value: Duration) -> Self` — Sets the trigger base poll interval.
- pub `trigger_poll_timeout` function L428-431 — `(mut self, value: Duration) -> Self` — Sets the trigger poll timeout.
- pub `enable_registry_reconciler` function L434-437 — `(mut self, value: bool) -> Self` — Enables or disables the registry reconciler.
- pub `registry_reconcile_interval` function L440-443 — `(mut self, value: Duration) -> Self` — Sets the registry reconcile interval.
- pub `registry_enable_startup_reconciliation` function L446-449 — `(mut self, value: bool) -> Self` — Enables or disables startup reconciliation.
- pub `registry_storage_path` function L452-455 — `(mut self, value: Option<std::path::PathBuf>) -> Self` — Sets the registry storage path.
- pub `registry_storage_backend` function L458-461 — `(mut self, value: impl Into<String>) -> Self` — Sets the registry storage backend.
- pub `require_signatures` function L466-469 — `(mut self, value: bool) -> Self` — CLOACI-T-0571: enable the reconciler's defense-in-depth
- pub `verification_org_id` function L475-478 — `(mut self, value: Option<crate::UniversalUuid>) -> Self` — CLOACI-T-0571: trusted org UUID forwarded to the reconciler so
- pub `runner_id` function L481-484 — `(mut self, value: Option<String>) -> Self` — Sets the runner identifier.
- pub `runner_name` function L487-490 — `(mut self, value: Option<String>) -> Self` — Sets the runner name.
- pub `default_executor` function L494-497 — `(mut self, value: impl Into<String>) -> Self` — Sets the executor key every task is dispatched to (CLOACI-T-0640).
- pub `enable_claiming` function L500-503 — `(mut self, value: bool) -> Self` — Enables or disables task claiming for horizontal scaling.
- pub `heartbeat_interval` function L506-509 — `(mut self, value: Duration) -> Self` — Sets the heartbeat interval for claimed tasks.
- pub `build` function L514-541 — `(self) -> Result<DefaultRunnerConfig, ConfigError>` — Builds and validates the configuration.
- pub `DefaultRunnerBuilder` struct L578-584 — `{ database_url: Option<String>, schema: Option<String>, config: DefaultRunnerCon...` — Builder for creating a DefaultRunner with PostgreSQL schema-based multi-tenancy
- pub `new` function L594-602 — `() -> Self` — Creates a new builder with default configuration
- pub `database_url` function L605-608 — `(mut self, url: &str) -> Self` — Sets the database URL
- pub `schema` function L614-617 — `(mut self, schema: &str) -> Self` — Sets the PostgreSQL schema for multi-tenant isolation
- pub `with_config` function L620-623 — `(mut self, config: DefaultRunnerConfig) -> Self` — Sets the full configuration
- pub `runtime` function L630-633 — `(mut self, runtime: Runtime) -> Self` — Sets a scoped [`Runtime`] for this runner.
- pub `runtime_arc` function L641-644 — `(mut self, runtime: Arc<Runtime>) -> Self` — Use an existing shared [`Arc<Runtime>`] for this runner.
- pub `build` function L658-770 — `(self) -> Result<DefaultRunner, WorkflowExecutionError>` — Builds the DefaultRunner
- pub `default_executor` function L786-789 — `(mut self, executor: impl Into<String>) -> Self` — Sets the executor key every task is dispatched to (CLOACI-T-0640).
-  `DefaultRunnerConfig` type L111-273 — `= DefaultRunnerConfig` — configuring the DefaultRunner's behavior.
-  `DefaultRunnerConfigBuilder` type L290-328 — `impl Default for DefaultRunnerConfigBuilder` — configuring the DefaultRunner's behavior.
-  `default` function L291-327 — `() -> Self` — configuring the DefaultRunner's behavior.
-  `DefaultRunnerConfigBuilder` type L330-542 — `= DefaultRunnerConfigBuilder` — configuring the DefaultRunner's behavior.
-  `DefaultRunnerConfig` type L544-550 — `impl Default for DefaultRunnerConfig` — configuring the DefaultRunner's behavior.
-  `default` function L545-549 — `() -> Self` — configuring the DefaultRunner's behavior.
-  `DefaultRunnerBuilder` type L586-590 — `impl Default for DefaultRunnerBuilder` — configuring the DefaultRunner's behavior.
-  `default` function L587-589 — `() -> Self` — configuring the DefaultRunner's behavior.
-  `DefaultRunnerBuilder` type L592-790 — `= DefaultRunnerBuilder` — configuring the DefaultRunner's behavior.
-  `validate_schema_name` function L647-655 — `(schema: &str) -> Result<(), WorkflowExecutionError>` — Validates the schema name contains only alphanumeric characters and underscores
-  `tests` module L793-968 — `-` — configuring the DefaultRunner's behavior.
-  `test_default_runner_config` function L797-812 — `()` — configuring the DefaultRunner's behavior.
-  `test_registry_storage_backend_configuration` function L815-841 — `()` — configuring the DefaultRunner's behavior.
-  `test_runner_identification` function L844-853 — `()` — configuring the DefaultRunner's behavior.
-  `test_registry_configuration_options` function L856-880 — `()` — configuring the DefaultRunner's behavior.
-  `test_cron_configuration` function L883-899 — `()` — configuring the DefaultRunner's behavior.
-  `test_db_pool_size_default` function L902-905 — `()` — configuring the DefaultRunner's behavior.
-  `test_config_clone` function L908-921 — `()` — configuring the DefaultRunner's behavior.
-  `test_config_debug` function L924-932 — `()` — configuring the DefaultRunner's behavior.
-  `test_builder_all_fields` function L935-967 — `()` — configuring the DefaultRunner's behavior.
...
- pub `with_database` function L131-181 — `( database: Database, config: DefaultRunnerConfig, shared_runtime: Option<Arc<Ru...` — CLOACI-T-0580: construct a runner around a pre-built `Database`,
- pub `database` function L184-186 — `(&self) -> &Database` — Returns a reference to the database.
- pub `dal` function L189-191 — `(&self) -> DAL` — Returns the DAL for database operations.
- pub `register_executor` function L202-209 — `(&self, key: &str, executor: Arc<dyn TaskExecutor>) -> bool` — Register an additional `TaskExecutor` on the runner's dispatcher under
- pub `has_executor` function L215-220 — `(&self, key: &str) -> bool` — Returns `true` if an executor is registered under `key` on this runner's
- pub `runtime` function L223-225 — `(&self) -> Arc<Runtime>` — Returns a handle to the scoped `Runtime` this runner uses.
- pub `unified_scheduler` function L228-230 — `(&self) -> Option<Arc<Scheduler>>` — Returns the unified scheduler if enabled.
- pub `set_graph_scheduler` function L234-240 — `( &self, scheduler: Arc<crate::computation_graph::scheduler::ComputationGraphSch...` — Set the graph scheduler for computation graph package routing.
- pub `shutdown` function L243-248 — `(&self) -> Result<(), WorkflowExecutionError>` — Gracefully shuts down the executor and its background services.
...
-  `DefaultRunner` type L79-249 — `= DefaultRunner` — - `DefaultRunnerBuilder`: Builder for creating runners with custom settings
-  `DefaultRunner` type L251-261 — `impl Clone for DefaultRunner` — - `DefaultRunnerBuilder`: Builder for creating runners with custom settings
-  `clone` function L252-260 — `(&self) -> Self` — - `DefaultRunnerBuilder`: Builder for creating runners with custom settings
-  `DefaultRunner` type L264-270 — `impl Drop for DefaultRunner` — - `DefaultRunnerBuilder`: Builder for creating runners with custom settings
-  `drop` function L265-269 — `(&mut self)` — - `DefaultRunnerBuilder`: Builder for creating runners with custom settings
...
#### crates/cloacina/tests/integration/dal/delivery_relay.rs

-  `CollectingSink` struct L37-39 — `{ seen: Mutex<Vec<i64>> }` — relay — proving the cross-replica path, not just an in-process channel.
-  `CollectingSink` type L42-47 — `impl DeliverySink for CollectingSink` — relay — proving the cross-replica path, not just an in-process channel.
-  `deliver` function L43-46 — `(&self, row: &DeliveryOutbox) -> Result<DeliveryOutcome, DeliveryError>` — relay — proving the cross-replica path, not just an in-process channel.
-  `test_notify_wakes_relay_across_connections` function L51-114 — `()` — relay — proving the cross-replica path, not just an in-process channel.

#### crates/cloacina/tests/integration/dal/mod.rs

- pub `api_keys` module L17 — `-`
- pub `context` module L18 — `-`
- pub `delivery_relay` module L20 — `-`
- pub `execution_events` module L21 — `-`
- pub `reactor_subscriptions` module L22 — `-`
- pub `reconciler_e2e_load` module L23 — `-`
- pub `sub_status` module L24 — `-`
- pub `task_claiming` module L25 — `-`
- pub `workflow_packages` module L26 — `-`
- pub `workflow_registry` module L27 — `-`
- pub `workflow_registry_reconciler_integration` module L28 — `-`
...
-  `test_predicate_filters_dispatch_and_advances_watermark_for_skips` function L177-329 — `()` — End-to-end: subscribe with a CEL filter, insert two firings (one
...
### crates/cloacina-agent/src

> *Semantic summary to be generated by AI agent.*

#### crates/cloacina-agent/src/main.rs
...
-  `synthetic_package_metadata` function L780-795 — `( digest: &str, ) -> cloacina::registry::loader::package_loader::PackageMetadata` — Build a minimal `PackageMetadata` for `TaskRegistrar::register_package_tasks`.
-  `build_context` function L799-817 — `( value: &serde_json::Value, ) -> Result<cloacina::Context<serde_json::Value>, a...` — Convert the work packet's `serde_json::Value` (expected to be an Object or
-  `kind_of` function L819-828 — `(v: &serde_json::Value) -> &'static str` — execute, classify the outcome, return `Success`/`Failure`.
-  `context_to_value` function L833-840 — `(ctx: &cloacina::Context<serde_json::Value>) -> serde_json::Value` — Materialize a `Context<serde_json::Value>` back into a JSON object so the
-  `ws_url_for` function L846-862 — `(server: &str, agent_id: &str, ticket: &str) -> Result<String>` — execute, classify the outcome, return `Success`/`Failure`.
-  `truncate` function L864-870 — `(s: &str, max: usize) -> &str` — execute, classify the outcome, return `Success`/`Failure`.
-  `_` variable L873-876 — `: fn()` — execute, classify the outcome, return `Success`/`Failure`.
-  `_assert_serialize` function L874 — `()` — execute, classify the outcome, return `Success`/`Failure`.
-  `tests` module L879-1013 — `-` — execute, classify the outcome, return `Success`/`Failure`.
-  `pkt` function L883-899 — `(triple: &str) -> WorkPacket` — execute, classify the outcome, return `Success`/`Failure`.
-  `target_triple_mismatch_refuses_pre_fetch` function L906-930 — `()` — execute, classify the outcome, return `Success`/`Failure`.
-  `artifact_fetch_failure_refuses` function L933-955 — `()` — execute, classify the outcome, return `Success`/`Failure`.
-  `tempdir` function L957-961 — `() -> std::path::PathBuf` — execute, classify the outcome, return `Success`/`Failure`.
-  `rand_suffix` function L963-970 — `() -> String` — execute, classify the outcome, return `Success`/`Failure`.
-  `build_context_accepts_object_or_null` function L973-979 — `()` — execute, classify the outcome, return `Success`/`Failure`.
-  `build_context_rejects_non_object_non_null` function L982-985 — `()` — execute, classify the outcome, return `Success`/`Failure`.
-  `context_to_value_round_trips_through_build_context` function L988-993 — `()` — execute, classify the outcome, return `Success`/`Failure`.
...
- pub `ListResponse` struct L26-29 — `{ items: Vec<T>, total: usize }` — Unified list envelope (CLOACI-T-0594 / API-03): every list endpoint
- pub `new` function L33-36 — `(items: Vec<T>) -> Self` — Build the envelope with `total` set to the page size.
...
- pub `ErrorBody` struct L26-32 — `{ error: String, code: String }` — Standardized error response body.

#### crates/cloacina-api-types/src/executions.rs

- pub `ExecuteRequest` struct L24-28 — `{ context: Option<serde_json::Value> }` — Request body for `POST /tenants/{tenant_id}/workflows/{name}/execute`.
- pub `ExecuteResponse` struct L33-40 — `{ execution_id: String, workflow_name: String, tenant_id: String, status: String...` — `202 Accepted` body for a scheduled workflow execution.
...
- pub `GraphStatus` struct L35-44 — `{ name: String, health: serde_json::Value, accumulators: Vec<String>, paused: bo...` — One row in `GET /v1/health/graphs`, and the `GET /v1/health/graphs/{name}`

#### crates/cloacina-api-types/src/keys.rs

- pub `KeyRole` enum L25-30 — `Admin | Write | Read` — Allowed roles for API keys.
- pub `as_str` function L33-39 — `(&self) -> &'static str` — API key management types.
- pub `CreateKeyRequest` struct L45-49 — `{ name: String, role: KeyRole }` — Request body for `POST /auth/keys` and `POST /tenants/{tenant_id}/keys`.
- pub `KeyCreatedResponse` struct L55-69 — `{ id: String, name: String, key: String, permissions: String, tenant_id: Option<...` — `201 Created` body for a new API key.
- pub `KeyInfo` struct L74-86 — `{ id: String, name: String, permissions: String, tenant_id: Option<String>, is_a...` — One row in the key list (`GET /auth/keys`).
- pub `KeyRevokedResponse` struct L91-96 — `{ status: String, id: String }` — `DELETE /auth/keys/{key_id}` response.
- pub `WsTicketResponse` struct L102-105 — `{ ticket: String, expires_in_seconds: u64 }` — `POST /auth/ws-ticket` response — a single-use, short-lived ticket for
-  `KeyRole` type L32-40 — `= KeyRole` — API key management types.
...
- pub `ReactorCommand` enum L30-36 — `ForceFire | FireWith | GetState | Pause | Resume` — Commands sent by WebSocket operators to a reactor.
- pub `ReactorResponse` enum L42-48 — `Fired | State | Paused | Resumed | Error` — Responses sent back to WebSocket operators.

#### crates/cloacina-api-types/src/tenants.rs

- pub `CreateTenantRequest` struct L24-34 — `{ name: String, description: Option<String>, password: Option<String> }` — Request body for `POST /tenants`.
- pub `TenantCreatedResponse` struct L40-44 — `{ name: String, username: String, description: Option<String> }` — `201 Created` body for a new tenant.
- pub `TenantRemovedResponse` struct L49-57 — `{ status: String, schema_name: String, revoked_keys: usize, runner_evicted: bool...` — `DELETE /tenants/{schema_name}` response — orchestrated teardown report.
- pub `TenantSummary` struct L62-64 — `{ name: String }` — One row in the tenant list (`GET /tenants`).
...
- pub `WorkflowUploadedResponse` struct L25-29 — `{ package_id: String, tenant_id: String }` — `201 Created` body for a workflow package upload
- pub `WorkflowSummary` struct L34-44 — `{ id: String, package_name: String, version: String, description: Option<String>...` — One row in the workflow list (`GET /tenants/{tenant_id}/workflows`).
- pub `WorkflowDeletedResponse` struct L49-54 — `{ status: String, package_name: String, version: String }` — `DELETE /tenants/{tenant_id}/workflows/{name}/{version}` response.
- pub `WorkflowDetail` struct L60-73 — `{ tenant_id: String, id: String, package_name: String, version: String, descript...` — `GET /tenants/{tenant_id}/workflows/{name}` response — summary fields
...
#### crates/cloacina-server/src/agent_registry.rs

- pub `AgentRecord` struct L30-41 — `{ agent_id: String, max_concurrency: u32, in_flight: u32, available_capacity: u3...` — Snapshot of a registered agent.
- pub `AgentRegistry` struct L44-46 — `{ by_id: Mutex<HashMap<String, AgentRecord>> }` — liveness sweeping lands in T-0634.
- pub `new` function L49-51 — `() -> Self` — liveness sweeping lands in T-0634.
- pub `register` function L55-58 — `(&self, record: AgentRecord)` — Insert or overwrite an entry (overwrite handles re-registration cleanly
- pub `record_heartbeat` function L63-78 — `( &self, agent_id: &str, in_flight: u32, available_capacity: u32, ) -> bool` — Update an existing entry's heartbeat fields.
- pub `deregister` function L81-84 — `(&self, agent_id: &str)` — Remove an entry.
- pub `sweep_dead` function L95-110 — `(&self, timeout: Duration) -> Vec<AgentRecord>` — Remove agents whose last heartbeat is older than `timeout` and return
- pub `snapshot` function L114-117 — `(&self) -> Vec<AgentRecord>` — Snapshot the current roster.
- pub `len` function L119-121 — `(&self) -> usize` — liveness sweeping lands in T-0634.
- pub `is_empty` function L123-125 — `(&self) -> bool` — liveness sweeping lands in T-0634.
-  `AgentRegistry` type L48-126 — `= AgentRegistry` — liveness sweeping lands in T-0634.
-  `tests` module L129-224 — `-` — liveness sweeping lands in T-0634.
-  `rec` function L132-143 — `(id: &str, cap: u32, tenant: Option<&str>) -> AgentRecord` — liveness sweeping lands in T-0634.
-  `register_then_snapshot_roundtrips` function L146-158 — `()` — liveness sweeping lands in T-0634.
-  `heartbeat_updates_capacity` function L161-168 — `()` — liveness sweeping lands in T-0634.
-  `heartbeat_on_unknown_agent_returns_false` function L171-174 — `()` — liveness sweeping lands in T-0634.
-  `sweep_dead_removes_only_stale_agents` function L177-193 — `()` — liveness sweeping lands in T-0634.
-  `sweep_dead_noop_when_all_fresh` function L196-203 — `()` — liveness sweeping lands in T-0634.
-  `deregister_is_idempotent` function L206-212 — `()` — liveness sweeping lands in T-0634.
-  `register_same_id_overwrites` function L215-223 — `()` — liveness sweeping lands in T-0634.

#### crates/cloacina-server/src/delivery_sink.rs

- pub `WsDeliverySink` struct L54-56 — `{ by_key: Mutex<HashMap<Key, mpsc::Sender<ServerMessage>>> }` — In-memory registry of `(recipient, tenant_id) → sender` for currently-
- pub `new` function L59-61 — `() -> Self` — in [[CLOACI-A-0006]] — no roster needed.
- pub `register` function L67-83 — `( &self, recipient: &str, tenant_id: Option<&str>, ) -> mpsc::Receiver<ServerMes...` — Register a recipient.
- pub `unregister` function L87-91 — `(&self, recipient: &str, tenant_id: Option<&str>)` — Deregister a recipient.
- pub `len` function L94-96 — `(&self) -> usize` — Current registry depth — useful for metrics (T-0628).
- pub `is_empty` function L98-100 — `(&self) -> bool` — in [[CLOACI-A-0006]] — no roster needed.
-  `CHANNEL_DEPTH` variable L45 — `: usize` — Per-connection mpsc channel depth.
-  `Key` type L49 — `= (String, Option<String>)` — Composite registry key.
-  `WsDeliverySink` type L58-101 — `= WsDeliverySink` — in [[CLOACI-A-0006]] — no roster needed.
-  `WsDeliverySink` type L104-127 — `impl DeliverySink for WsDeliverySink` — in [[CLOACI-A-0006]] — no roster needed.
-  `deliver` function L105-126 — `(&self, row: &DeliveryOutbox) -> Result<DeliveryOutcome, DeliveryError>` — in [[CLOACI-A-0006]] — no roster needed.
-  `tests` module L130-208 — `-` — in [[CLOACI-A-0006]] — no roster needed.
-  `row` function L134-147 — `(recipient: &str, tenant: Option<&str>) -> DeliveryOutbox` — in [[CLOACI-A-0006]] — no roster needed.
-  `deliver_to_unregistered_recipient_is_no_route` function L150-154 — `()` — in [[CLOACI-A-0006]] — no roster needed.
-  `deliver_to_registered_recipient_pushes_frame` function L157-176 — `()` — in [[CLOACI-A-0006]] — no roster needed.
-  `tenant_isolation_recipient_string_alone_is_not_enough` function L179-193 — `()` — in [[CLOACI-A-0006]] — no roster needed.
-  `full_channel_returns_no_route_for_backpressure` function L196-207 — `()` — in [[CLOACI-A-0006]] — no roster needed.

#### crates/cloacina-server/src/fleet_coordinator.rs

- pub `FleetCoordinator` struct L44-46 — `{ pending: Mutex<HashMap<UniversalUuid, oneshot::Sender<AgentResultRequest>>> }` — cross-replica fan-out direction.
- pub `new` function L49-51 — `() -> Self` — cross-replica fan-out direction.
- pub `register_pending` function L57-71 — `( &self, task_execution_id: UniversalUuid, ) -> oneshot::Receiver<AgentResultReq...` — Register a pending result expectation.
- pub `forward` function L78-104 — `( &self, task_execution_id: UniversalUuid, result: AgentResultRequest, ) -> Resu...` — Forward an incoming agent result to the executor waiting on it.
- pub `cancel` function L108-111 — `(&self, task_execution_id: UniversalUuid)` — Drop a pending entry without sending.
- pub `pending_count` function L114-116 — `(&self) -> usize` — Pending count — useful for metrics/debug.
-  `FleetCoordinator` type L48-117 — `= FleetCoordinator` — cross-replica fan-out direction.
-  `tests` module L120-179 — `-` — cross-replica fan-out direction.
-  `synthetic_result` function L124-135 — `(task_id: UniversalUuid) -> AgentResultRequest` — cross-replica fan-out direction.
-  `register_then_forward_delivers_to_receiver` function L138-146 — `()` — cross-replica fan-out direction.
-  `forward_without_pending_is_err_orphan` function L149-155 — `()` — cross-replica fan-out direction.
-  `cancel_removes_pending_so_subsequent_forward_is_orphan` function L158-165 — `()` — cross-replica fan-out direction.
-  `pending_count_tracks_inserts_and_removals` function L168-178 — `()` — cross-replica fan-out direction.

#### crates/cloacina-server/src/fleet_executor.rs

- pub `FleetExecutor` struct L73-97 — `{ dal: cloacina::dal::DAL, agent_registry: Arc<AgentRegistry>, coordinator: Arc<...` — and filters by tenant.
- pub `new` function L100-120 — `( dal: cloacina::dal::DAL, agent_registry: Arc<AgentRegistry>, coordinator: Arc<...` — and filters by tenant.
- pub `with_name` function L122-125 — `(mut self, name: impl Into<String>) -> Self` — and filters by tenant.
-  `RESULT_WAIT_TIMEOUT` variable L62 — `: Duration` — Default ceiling on how long the executor will wait for an agent to report
-  `MIN_ADVERTISED_CAPACITY` variable L66 — `: usize` — Baseline advertised capacity so an empty fleet still reports a value the
...
- pub `agent_registry` module L23 — `-` — Cloacina HTTP API server library.
- pub `delivery_sink` module L24 — `-` — management, workflow upload, and execution APIs.
- pub `fleet_coordinator` module L25 — `-` — management, workflow upload, and execution APIs.
- pub `fleet_executor` module L26 — `-` — management, workflow upload, and execution APIs.
- pub `openapi` module L27 — `-` — management, workflow upload, and execution APIs.
- pub `routes` module L28 — `-` — management, workflow upload, and execution APIs.
- pub `tenant_runner_cache` module L29 — `-` — management, workflow upload, and execution APIs.
- pub `CorsConfig` struct L35-42 — `{ allowed_origins: Vec<String>, allowed_methods: Vec<String>, allowed_headers: V...` — CORS configuration (CLOACI-T-0643 / REQ-009).
- pub `layer` function L47-107 — `(&self) -> Result<Option<tower_http::cors::CorsLayer>>` — Build the tower-http layer, or `None` when CORS is disabled.
- pub `TenantDatabaseCache` struct L128-131 — `{ databases: tokio::sync::RwLock<std::collections::HashMap<String, Database>>, d...` — Cached per-tenant database connections for schema isolation.
- pub `new` function L134-139 — `(database_url: String) -> Self` — management, workflow upload, and execution APIs.
- pub `resolve` function L144-176 — `( &self, tenant_id: &str, admin_db: &Database, ) -> Result<Database, cloacina::d...` — Get or create a schema-scoped Database for the given tenant.
- pub `evict` function L182-185 — `(&self, tenant_id: &str) -> bool` — CLOACI-T-0581: drop the cached `Database` for a tenant.
- pub `AppState` struct L190-235 — `{ database: Database, runner: Arc<DefaultRunner>, key_cache: Arc<crate::routes::...` — Shared application state accessible from all route handlers.
- pub `run` function L285-1049 — `( home: std::path::PathBuf, bind: SocketAddr, database_url: String, verbose: boo...` — Run the API server.
-  `CorsConfig` type L44-108 — `= CorsConfig` — management, workflow upload, and execution APIs.
-  `TenantDatabaseCache` type L133-186 — `= TenantDatabaseCache` — management, workflow upload, and execution APIs.
...
-  `Cli` struct L30-154 — `{ verbose: bool, home: PathBuf, bind: SocketAddr, database_url: Option<String>, ...` — cloacina-server — HTTP API for Cloacina, backed by Postgres.
-  `Command` enum L157-162 — `EmitOpenapi` — command in T-0510 (CLOACI-I-0098).
-  `default_home` function L164-168 — `() -> PathBuf` — command in T-0510 (CLOACI-I-0098).
-  `main` function L171-209 — `() -> Result<()>` — command in T-0510 (CLOACI-I-0098).
...
- pub `ApiDoc` struct L160 — `-` — OpenAPI cannot describe WS message flows.
- pub `openapi_json` function L165-169 — `() -> String` — Pretty-printed OpenAPI document.
...
-  `tests` module L172-190 — `-` — OpenAPI cannot describe WS message flows.
-  `openapi_document_builds_and_serializes` function L176-181 — `()` — OpenAPI cannot describe WS message flows.
-  `spec_version_matches_crate_version` function L184-189 — `()` — OpenAPI cannot describe WS message flows.
...
- pub `FleetRegistrar` type L86 — `= Arc<dyn Fn(&DefaultRunner) + Send + Sync>` — Hook invoked on each freshly-built per-tenant runner to register the fleet
- pub `TenantRunnerCache` struct L89-107 — `{ cache: Mutex<LruCache<String, Arc<DefaultRunner>>>, shared_runtime: Arc<Runtim...` — LRU-bounded cache of per-tenant `DefaultRunner` instances.
- pub `new` function L113-121 — `(capacity: NonZeroUsize, base_config: DefaultRunnerConfig) -> Self` — Build a new cache with the given LRU cap.
- pub `with_fleet_registrar` function L128-131 — `(mut self, registrar: FleetRegistrar) -> Self` — CLOACI-T-0634: install a fleet-executor registrar.
- pub `with_graph_scheduler` function L139-142 — `(mut self, scheduler: Arc<ComputationGraphScheduler>) -> Self` — CLOACI-T-0581 follow-up: install a shared graph scheduler.
- pub `shared_runtime` function L146-148 — `(&self) -> Arc<Runtime>` — Get the shared `Runtime` so callers can install graph schedulers,
- pub `get_or_create` function L160-232 — `( &self, tenant_id: &str, tenant_database: Database, ) -> Result<Arc<DefaultRunn...` — Look up (or construct) the runner for `tenant_id`, bound to
- pub `evict` function L238-254 — `(&self, tenant_id: &str) -> Result<bool, cloacina::WorkflowExecutionError>` — Explicitly evict a tenant's runner from the cache, awaiting its
- pub `evict_with_timeout` function L260-300 — `( &self, tenant_id: &str, drain_timeout: std::time::Duration, ) -> EvictOutcome` — CLOACI-T-0581: bounded-drain eviction.
- pub `shutdown_all` function L305-320 — `(&self) -> HashMap<String, Result<(), String>>` — Shut down every cached runner.
- pub `len` function L323-325 — `(&self) -> usize` — Current number of cached runners.
- pub `is_empty` function L328-330 — `(&self) -> bool` — `true` if the cache holds no runners.
-  `EvictOutcome` type L70-76 — `= EvictOutcome` — next cache lookup.
-  `TenantRunnerCache` type L109-331 — `= TenantRunnerCache` — next cache lookup.
-  `tests` module L334-363 — `-` — next cache lookup.
-  `cap` function L337-339 — `(n: usize) -> NonZeroUsize` — next cache lookup.
-  `empty_cache_is_empty` function L342-346 — `()` — next cache lookup.
-  `evict_missing_tenant_returns_false` function L349-353 — `()` — next cache lookup.
-  `shared_runtime_is_stable_arc` function L356-362 — `()` — next cache lookup.

### crates/cloacina-server/src/routes

> *Semantic summary to be generated by AI agent.*

#### crates/cloacina-server/src/routes/agent.rs

- pub `register_agent` function L67-107 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, J...` — `POST /v1/agent/register`
...
-  `authenticate_substrate_ws` function L73-100 — `( state: &AppState, headers: &HeaderMap, query: &WsAuthQuery, ) -> Result<Authen...` — Token extraction + validation.
-  `handle_delivery_socket` function L103-197 — `( mut socket: WebSocket, recipient: String, tenant: Option<String>, dal: DAL, si...` — Per-connection state machine.
-  `send_server_frame` function L201-215 — `(socket: &mut WebSocket, msg: &ServerMessage) -> bool` — Serialize and send a `ServerMessage` as a text frame.
-  `FrameOutcome` enum L219-222 — `Ok | Close` — Result of processing one client frame: keep going, or close with a
-  `handle_client_frame` function L225-275 — `(text: &str, dal: &DAL) -> FrameOutcome` — Parse and act on one client frame.
...
- pub `list_graphs` function L114-143 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, )...` — registered-but-not-running graphs.
- pub `get_graph` function L160-194 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — registered-but-not-running graphs.
...
-  `tests` module L197-233 — `-` — registered-but-not-running graphs.
-  `auth` function L200-208 — `(tenant: Option<&str>, is_admin: bool) -> AuthenticatedKey` — registered-but-not-running graphs.
-  `graph_visible_admin_sees_all` function L211-216 — `()` — registered-but-not-running graphs.
-  `graph_visible_tenant_scoped` function L219-225 — `()` — registered-but-not-running graphs.
-  `graph_visible_global_key_cannot_see_tenant_graphs` function L228-232 — `()` — registered-but-not-running graphs.

#### crates/cloacina-server/src/routes/keys.rs

- pub `create_key` function L57-112 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, J...` — The bootstrap key is created automatically on first server startup.
- pub `list_keys` function L128-158 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, )...` — The bootstrap key is created automatically on first server startup.
- pub `revoke_key` function L176-210 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — The bootstrap key is created automatically on first server startup.
- pub `create_tenant_key` function L228-276 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — The bootstrap key is created automatically on first server startup.
- pub `create_ws_ticket` function L292-301 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, )...` — The bootstrap key is created automatically on first server startup.
...
- pub `create_tenant` function L56-101 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, J...` — Tenants are isolated Postgres schemas.
- pub `remove_tenant` function L135-248 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — Tenants are isolated Postgres schemas.
- pub `list_tenants` function L264-290 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, )...` — Tenants are isolated Postgres schemas.
...
- pub `upload_workflow` function L60-212 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — Workflow package API — upload/list/get/delete .cloacina packages per tenant.
- pub `list_workflows` function L228-274 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — Workflow package API — upload/list/get/delete .cloacina packages per tenant.
- pub `get_workflow` function L294-390 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — Workflow package API — upload/list/get/delete .cloacina packages per tenant.
- pub `delete_workflow` function L411-463 — `( State(state): State<AppState>, Extension(auth): Extension<AuthenticatedKey>, P...` — Workflow package API — upload/list/get/delete .cloacina packages per tenant.
-  `extract_file_field` function L466-477 — `(multipart: &mut Multipart) -> Result<Vec<u8>, String>` — Extract the first file field from a multipart request.
...
- pub `CloacinaConfig` struct L33-59 — `{ database_url: Option<String>, default_profile: Option<String>, profiles: BTree...` — Full configuration file structure.
- pub `Profile` struct L64-70 — `{ server: String, api_key: String }` — A named server-targeting profile.
- pub `DaemonSection` struct L74-92 — `{ poll_interval_ms: u64, log_level: String, shutdown_timeout_s: u64, watcher_deb...` — - Config value lookup for commands that need database_url etc.
- pub `CompilerSection` struct L111-115 — `{ local_addr: String }` — - Config value lookup for commands that need database_url etc.
- pub `WatchSection` struct L127-129 — `{ directories: Vec<String> }` — - Config value lookup for commands that need database_url etc.
- pub `ServerSection` struct L133-140 — `{ default_executor: String }` — - Config value lookup for commands that need database_url etc.
- pub `load` function L153-182 — `(path: &Path) -> Self` — Load config from a TOML file.
- pub `save` function L185-195 — `(&self, path: &Path) -> Result<()>` — Save config to a TOML file.
- pub `resolve_watch_dirs` function L198-211 — `(&self) -> Vec<PathBuf>` — Resolve watch directories from config, expanding `~` to home dir.
- pub `get` function L214-218 — `(&self, key: &str) -> Option<String>` — Get a config value by dotted key path (e.g., "daemon.poll_interval_ms").
- pub `set` function L221-233 — `(&mut self, key: &str, value: &str) -> Result<()>` — Set a config value by dotted key path.
- pub `list` function L236-244 — `(&self) -> Vec<(String, String)>` — List all config key-value pairs.
- pub `run_get` function L342-353 — `(config_path: &Path, key: &str) -> Result<()>` — Run `cloacinactl config get <key>`.
- pub `run_set` function L356-362 — `(config_path: &Path, key: &str, value: &str) -> Result<()>` — Run `cloacinactl config set <key> <value>`.
- pub `run_list` function L365-376 — `(config_path: &Path) -> Result<()>` — Run `cloacinactl config list`.
- pub `run_profile_set` function L379-401 — `( config_path: &Path, name: &str, server: &str, api_key: &str, default: bool, ) ...` — Run `cloacinactl config profile set <NAME> <URL> --api-key <K> [--default]`.
- pub `run_profile_list` function L404-421 — `(config_path: &Path) -> Result<()>` — Run `cloacinactl config profile list`.
- pub `run_profile_use` function L424-433 — `(config_path: &Path, name: &str) -> Result<()>` — Run `cloacinactl config profile use <NAME>`.
- pub `run_profile_delete` function L436-447 — `(config_path: &Path, name: &str) -> Result<()>` — Run `cloacinactl config profile delete <NAME>`.
- pub `resolve_database_url` function L464-478 — `(cli_url: Option<&str>, config_path: &Path) -> Result<String>` — Resolve database_url from CLI arg or config file.
- pub `resolve_default_executor` function L487-492 — `(cli_value: Option<&str>, config_path: &Path) -> String` — Resolve the server's default executor from CLI arg or config file
-  `DaemonSection` type L94-107 — `impl Default for DaemonSection` — - Config value lookup for commands that need database_url etc.
-  `default` function L95-106 — `() -> Self` — - Config value lookup for commands that need database_url etc.
-  `CompilerSection` type L117-123 — `impl Default for CompilerSection` — - Config value lookup for commands that need database_url etc.
-  `default` function L118-122 — `() -> Self` — - Config value lookup for commands that need database_url etc.
-  `ServerSection` type L142-148 — `impl Default for ServerSection` — - Config value lookup for commands that need database_url etc.
-  `default` function L143-147 — `() -> Self` — - Config value lookup for commands that need database_url etc.
-  `CloacinaConfig` type L150-245 — `= CloacinaConfig` — - Config value lookup for commands that need database_url etc.
-  `resolve_key` function L248-255 — `(value: &'a toml::Value, key: &str) -> Option<&'a toml::Value>` — Resolve a dotted key path in a TOML value tree.
-  `set_key` function L258-304 — `(root: &mut toml::Value, key: &str, value: &str) -> Result<()>` — Set a value at a dotted key path in a TOML value tree.
-  `collect_pairs` function L307-323 — `(value: &toml::Value, prefix: &str, pairs: &mut Vec<(String, String)>)` — Collect all leaf key-value pairs with dotted paths.
-  `format_value` function L326-339 — `(value: &toml::Value) -> String` — Format a TOML value for display.
-  `redact_secret` function L453-461 — `(raw: &str) -> String` — Short redacted form of a secret for display.
-  `tests` module L495-660 — `-` — - Config value lookup for commands that need database_url etc.
-  `config_defaults_are_sensible` function L500-513 — `()` — - Config value lookup for commands that need database_url etc.
-  `resolve_default_executor_precedence` function L516-529 — `()` — - Config value lookup for commands that need database_url etc.
-  `config_load_missing_file_returns_defaults` function L532-536 — `()` — - Config value lookup for commands that need database_url etc.
-  `config_load_valid_toml` function L539-567 — `()` — - Config value lookup for commands that need database_url etc.
-  `config_load_invalid_toml_returns_defaults` function L570-579 — `()` — - Config value lookup for commands that need database_url etc.
-  `config_load_partial_toml_fills_defaults` function L582-592 — `()` — - Config value lookup for commands that need database_url etc.
-  `config_resolve_watch_dirs_expands_tilde` function L595-606 — `()` — - Config value lookup for commands that need database_url etc.
-  `config_resolve_watch_dirs_empty` function L609-612 — `()` — - Config value lookup for commands that need database_url etc.
-  `config_save_and_reload_roundtrip` function L615-631 — `()` — - Config value lookup for commands that need database_url etc.
-  `config_get_dotted_key` function L634-642 — `()` — - Config value lookup for commands that need database_url etc.
-  `config_set_dotted_key` function L645-649 — `()` — - Config value lookup for commands that need database_url etc.
-  `config_list_returns_all_keys` function L652-659 — `()` — - Config value lookup for commands that need database_url etc.
...
- pub `run` function L71-94 — `(self, globals: &GlobalOpts) -> Result<()>` — `cloacinactl server <verb>` — cloacina-server HTTP API verbs.
-  `ServerVerb` enum L37-68 — `Start | Stop | Status | Health` — `cloacinactl server <verb>` — cloacina-server HTTP API verbs.
-  `ServerCmd` type L70-95 — `= ServerCmd` — `cloacinactl server <verb>` — cloacina-server HTTP API verbs.

#### crates/cloacinactl/src/nouns/server/start.rs

- pub `run` function L26-68 — `( globals: &GlobalOpts, bind: SocketAddr, database_url: Option<String>, bootstra...`
-  `_config_type_check` function L72 — `(_: CloacinaConfig)`
...
#### examples/fixtures/fleet-slow-rust/build.rs

-  `main` function L17-19 — `()`

### examples/fixtures/fleet-slow-rust/src

> *Semantic summary to be generated by AI agent.*

#### examples/fixtures/fleet-slow-rust/src/lib.rs

- pub `fleet_slow_workflow` module L32-60 — `-`
- pub `slow` function L43-59 — `(context: &mut Context<serde_json::Value>) -> Result<(), TaskError>`
