...
**REMAINING (packaged-Python lane, = T-0836's Python leg):** (c) python-load-path provider unpack (`set_provider_search_path` before module import in the reconciler's Python branch) + `[providers]` package.toml section driving synthesized-Cargo.toml bundling in the compiler's Python branch → (d) packaged Python demo + server test.
