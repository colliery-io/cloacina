...
**Plan:** (1) `provider_bundle::pack_providers_from_specs(specs: Vec<(name, toml-dep-value)>)` — synthesize a scratch Cargo project (`[workspace]` empty to escape parent capture, stub lib.rs) with the dep specs VERBATIM (supports `"0.1"` / `{ path = "…" }` / `{ git = "…" }` uniformly) → `resolve_provider_crate`+`pack_providers` verbatim. (2) `CloacinaMetadata` (workflow-plugin, deny_unknown_fields) gains optional `providers: HashMap<String, ProviderDep>` (`#[serde(untagged)]` Version(String) | Detailed{version/path/git}) — additive, old packages unaffected. (3) compiler `run_build` python arm: metadata.providers non-empty → specs → pack → `store_package_providers`. (4) reconciler: extract step-5b's fetch+unpack+set into shared `stage_bundled_providers(&metadata)`; call it in the PYTHON branch before `load_workflow_package` (fail closed if rows exist but feature off). (5) packaged-Python e2e (python_package.rs precedent has the PythonRuntime harness).
