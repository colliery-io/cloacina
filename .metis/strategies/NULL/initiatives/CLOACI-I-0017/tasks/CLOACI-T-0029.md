---
id: split-database-connection-rs-into
level: task
...
# Split database/connection.rs into modules

## Objective

Split `src/database/connection.rs` (1,010 lines) into focused modules.
...
- Multi-backend abstraction (Postgres/SQLite)
- Type conversions (UniversalTypes)
...
- [ ] Create `pool.rs` with connection pool logic
- [ ] Create `backend.rs` with backend type detection
- [ ] Create `connection_builder.rs` with setup/migration logic
