...
**(6c) DONE ✅ — raw spike retired (human chose A: retire + migrate coverage), commit 1f1b0b2e.** Deleted `task-constructor-fixture` + `trigger-constructor-fixture` (raw hand-written glue), the redundant `constructor_loader_wasm.rs` (covered by `constructor_macro_wasm`), and the orphaned `examples/constructor-contract/host` spike harness (raw-fidius, not in CI, only consumer of the deleted raw fixture). Rewrote `constructor_trigger_wasm.rs` to keep its UNIQUE `load_constructor` runtime-registration coverage on the macro suite fixtures (heartbeat + prefix), packaged via the real `package_constructor_provider` path.

### ✅ FULL `constructors-wasm` LANE GREEN ON THE SUITE MODEL (8 files, 29 tests, 0 failed)
provider_package(5) · macro(4) · trigger_macro(3) · accumulator(4) · reactor(3) · reactor_scheduler(3) · workflow_node(2) · trigger(4). Run: `cargo test -p cloacina --features constructors-wasm --test constructor_provider_package_wasm --test constructor_macro_wasm --test constructor_trigger_macro_wasm --test constructor_accumulator_wasm --test constructor_reactor_wasm --test constructor_reactor_scheduler_wasm --test constructor_workflow_node_wasm --test constructor_trigger_wasm`.
