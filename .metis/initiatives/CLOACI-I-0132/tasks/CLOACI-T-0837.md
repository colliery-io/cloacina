...
**(6b) Other wasm tests** (~7): `constructor_macro_wasm`, `constructor_loader_wasm`, `constructor_trigger_macro_wasm`, `constructor_accumulator_wasm`, `constructor_reactor_wasm`, `constructor_reactor_scheduler_wasm`, `constructor_workflow_node_wasm`, `constructor_trigger_wasm`. Each: setup helper now stages `provider.json` (call the fixture's `__provider_manifest()` not `__constructor_manifest()`), and the `load_*` call gains `constructor_name` (= the fixture's member name: prefix/threshold/gate/affix/heartbeat). Loose-dir tests that hand-stage a package dir must write provider.json + set package.toml name = provider name.
