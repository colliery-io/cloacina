...
**REMAINING for T-0836:** (5) compiler `run_build` wiring — `discover_provider_refs(source_dir)` source scan → `bundle_providers` (packed archives) → `dal.workflow_packages().upsert_provider(...)` per provider (compiler needs cloacina's `constructor-packaging` feature; live verification = docker demo lane). (opt) DAL round-trip test on sqlite to run-verify migration 028. (then) the packaged Rust server demo + Python (T-0831).
