...
- **Testing strategy (2026-06-23):** OIDC integration + e2e tests run against a **dockerized Dex/Keycloak sidecar in the test compose** (the `angreal` integration lane) — no external IdP, no manual setup. Local-accounts and Phase 0 tests need no sidecar. The shared mint/refresh spine (T-0792/0793/0794) is exercised by the local path first, so it's covered even before the OIDC sidecar is wired.
- **Execution (2026-06-23): full linear ralph-tasks loop**, no-IdP-first order: `0782 → 0783 → 0784 → 0785 → 0786 → 0787 → 0792 → 0793 → 0794 → 0795 → 0796 → 0797 → 0798 → 0799 → 0788 → 0789 → 0790 → 0791`. T-0792 reframed provider-agnostic so it precedes OIDC cleanly. Phases 4–5 (GitHub, hardening/docs/ADR) + OQ-4 remain held.
