...
fn generate_vertical_ascii(tasks: &[TaskInfo], details: bool) -> Result<()> {
    ...
    for (i, task) in ordered_tasks.iter().enumerate() {
        let box_width = if details {
            40
        } else {
            std::cmp::max(task.id.len() + 4, 16)
        };
...
fn generate_horizontal_ascii_from_graph(
...
fn calculate_execution_levels_from_graph(
    dependencies: &std::collections::HashMap<String, Vec<String>>,
) -> Result<Vec<Vec<String>>> {
    let mut levels = Vec::new();
    let mut remaining: std::collections::HashSet<String> = dependencies.keys().cloned().collect();
...
fn generate_compact_ascii_from_graph(
    graph: &cloacina::WorkflowGraphData,
    details: bool,
) -> Result<()> {
    if graph.nodes.is_empty() {
