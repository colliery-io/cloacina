...
fn test_compile_command_functional() {
    ...
    let output = cmd.output().expect("Failed to execute cloacina-ctl compile");
...
fn test_package_command_functional() {
    ...
    let output = cmd.output().expect("Failed to execute cloacina-ctl package");
...
fn test_inspect_command_functional() {
    ...
    let package_output = package_cmd.output().expect("Failed to execute cloacina-ctl package");
    if !package_output.status.success() {
        eprintln!("Package creation failed");
        eprintln!("STDOUT: {}", String::from_utf8_lossy(&package_output.stdout));
        eprintln!("STDERR: {}", String::from_utf8_lossy(&package_output.stderr));
    ...
    let inspect_output = inspect_cmd.output().expect("Failed to execute cloacina-ctl inspect");
    
    if !inspect_output.status.success() {
        eprintln!("STDOUT: {}", String::from_utf8_lossy(&inspect_output.stdout));
        eprintln!("STDERR: {}", String::from_utf8_lossy(&inspect_output.stderr));
    ...
    if !stdout.trim().starts_with('{') && !stdout.trim().starts_with('[') {
        ...
        eprintln!("DEBUG - Inspect stderr: '{}'", String::from_utf8_lossy(&inspect_output.stderr));
    }
    
    assert!(stdout.contains("test_workflow"), "Output should contain workflow name");
    assert!(stdout.contains("first_task"), "Output should contain first task");
    assert!(stdout.contains("second_task"), "Output should contain second task");
    ...
    let _: serde_json::Value = serde_json::from_str(json_part)
...
fn test_debug_list_command_functional() {
    ...
    let debug_output = debug_cmd.output().expect("Failed to execute cloacina-ctl debug list");
    ...
    assert!(stdout.contains("first_task"), "Debug list should contain first task");
    assert!(stdout.contains("second_task"), "Debug list should contain second task");
    assert!(stdout.contains("0:") || stdout.contains("1:"), "Debug list should show task indices");
}
...
fn test_invalid_project_path() {
    ...
    assert!(!output.status.success(), "Command should fail for invalid project path");
    
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.to_lowercase().contains("error") || stderr.to_lowercase().contains("failed"), 
...
fn test_help_command() {
    ...
    assert!(stdout.contains("cloacina-ctl"), "Help should contain program name");
    assert!(stdout.contains("compile"), "Help should mention compile command");
    assert!(stdout.contains("package"), "Help should mention package command");
    assert!(stdout.contains("inspect"), "Help should mention inspect command");
    assert!(stdout.contains("visualize"), "Help should mention visualize command");
    assert!(stdout.contains("debug"), "Help should mention debug command");
}