...
use cloacina_ctl::manifest::{PackageManifest, PackageInfo, LibraryInfo, TaskInfo};
...
fn test_extract_manifest() {
    ...
    create_package_archive(&compile_result, &archive_path, &create_test_cli()).expect("Failed to create archive");
    
    // Extract and verify manifest
    let extracted_manifest = extract_manifest_from_package(&archive_path)
        .expect("Failed to extract manifest");
    
    assert_eq!(extracted_manifest.package.name, original_manifest.package.name);
    assert_eq!(extracted_manifest.package.version, original_manifest.package.version);
    assert_eq!(extracted_manifest.tasks.len(), original_manifest.tasks.len());
...
fn test_extract_library() {
    ...
    assert_eq!(extracted_content, library_content, "Library content should match");
}
...
fn test_archive_without_manifest() {
    ...
    header.set_cksum();
    tar_builder.append_data(&mut header, "dummy.txt", &dummy_data[..])
    ...
    assert!(error_msg.contains("manifest.json not found"), "Error should mention missing manifest");
}