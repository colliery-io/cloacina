...
| The graph to fire on every accumulator boundary, in-process minimum-latency | Standalone with `#[computation_graph(trigger = reactor("name"))]` — see [Tutorial 10]({{< ref "/embed/tutorials/10-computation-graph" >}}) |
...
- **Authoring the graph itself.** See [Tutorial 10 — Your First Computation Graph]({{< ref "/embed/tutorials/10-computation-graph" >}}).
