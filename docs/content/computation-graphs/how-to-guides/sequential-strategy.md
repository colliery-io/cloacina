...
- Familiarity with the reactor model (see [tutorial 09 — full pipeline]({{< ref "/computation-graphs/tutorials/library/09-full-pipeline" >}}))
...
The macro-level `#[reactor(...)]` form currently accepts `name`, `accumulators`, and `criteria` — there is no `input_strategy = sequential` clause. The strategy is set on the runtime `Reactor::new(...)` builder:
...
Sequential has higher per-event overhead than Latest: each boundary causes one full graph execution and one DAL persistence cycle. For high-throughput sources this can become a bottleneck. If you need guaranteed delivery at high volume, consider pairing Sequential with a [batch accumulator]({{< ref "accumulator-types" >}}#batch) to amortise cost: buffer many events into one boundary, then let Sequential ensure every batch is processed in order.

## Related

- [Choosing and using accumulator types]({{< ref "accumulator-types" >}})
- [How to use when_all reaction criteria]({{< ref "when-all-criteria" >}})
