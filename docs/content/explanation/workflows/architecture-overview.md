...
**DefaultRunner** is the orchestration entry point. It owns the database connection, configuration, and all background service handles. When you call `DefaultRunner::new()`, it initializes the database, runs migrations, creates the `TaskScheduler`, `ThreadTaskExecutor`, and `DefaultDispatcher`, then starts background services. It also implements the `PipelineExecutor` trait, which the `Scheduler` uses to hand off workflow executions.
...
**Unified Scheduler** (`Scheduler`) drives both cron and trigger-based workflow execution from a single polling loop. It queries the database for due cron schedules, atomically claims them to prevent duplicate execution across instances, polls registered triggers respecting per-trigger intervals, and deduplicates trigger firings based on context hashes. It follows a saga pattern: create an audit record, hand off to the `PipelineExecutor`, then link the result back.
