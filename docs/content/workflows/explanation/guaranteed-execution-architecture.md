...
sequenceDiagram
    ...
    participant PE as WorkflowExecutor
...
async fn recover_execution(&self, execution: &CronExecution) -> Result<(), WorkflowExecutionError> {
...
let cron_scheduler = CronScheduler::new(
    Arc::new(dal),
    Arc::new(self.clone()), // DefaultRunner implements WorkflowExecutor!
    cron_config,
    cron_shutdown_rx,
);
```
...
// From cron_recovery.rs
impl CronRecovery {
    pub async fn run(&self) -> Result<(), WorkflowExecutionError> {
...
```rust
async fn recover_lost_executions(&self) -> Result<(), WorkflowExecutionError> {
...
The separation between scheduling (CronScheduler) and execution (WorkflowExecutor) follows the saga pattern, where each service has clear responsibilities and compensation actions. This prevents the complexity that would arise from trying to handle both concerns in a single component.
