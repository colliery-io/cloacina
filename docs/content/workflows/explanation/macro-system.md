...
Task and workflow registration is handled automatically by `inventory::submit!` calls that the macros emit; `cloacina::Runtime::seed_from_inventory()` walks those entries at startup. Older docs may still reference a `ctor = "0.2"` dependency for "task registration"; that was the I-0096 predecessor and is no longer required (or used).
