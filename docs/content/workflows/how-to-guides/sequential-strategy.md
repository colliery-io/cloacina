...
Sequential has higher per-event overhead than Latest: each boundary causes one full graph execution and one DAL persistence cycle. For high-throughput sources this can become a bottleneck. If you need guaranteed delivery at high volume, consider pairing Sequential with a [batch accumulator]({{< ref "/computation-graphs/how-to-guides/accumulator-types" >}}#batch) to amortise cost: buffer many events into one boundary, then let Sequential ensure every batch is processed in order.
