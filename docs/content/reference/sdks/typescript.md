...
{{< hint type=note title="Not on the npm registry" >}}
`@cloacina/client` is **not published to npm** — it ships in-repo at
[`clients/typescript`](https://github.com/colliery-io/cloacina/tree/main/clients/typescript),
kept in lockstep with the server by a CI drift gate. Install it from the
repository:

```bash
git clone https://github.com/colliery-io/cloacina && cd cloacina/clients/typescript
npm ci && npm run build && npm pack   # yields cloacina-client-<version>.tgz
npm install /path/to/cloacina-client-<version>.tgz   # in your project
```
{{< /hint >}}
